...
import argparse  # 터미널에서 실행할 때 옵션(인자)을 받기 위한 라이브러리를 불러온다
import os  # 파일과 폴더를 다루기 위한 운영체제 기능 라이브러리를 불러온다
import sys  # 파이썬 실행 환경(경로 등)을 다루기 위한 라이브러리를 불러온다
import time  # 시간을 측정하기 위한 라이브러리를 불러온다
...
import numpy as np  # 숫자 배열 계산을 빠르게 할 수 있는 넘파이 라이브러리를 불러온다
import torch  # 딥러닝(인공지능 학습)을 위한 파이토치 라이브러리를 불러온다
import pandas as pd  # 표 형태의 데이터를 다루기 위한 판다스 라이브러리를 불러온다
from datetime import datetime, timezone, timedelta  # 날짜와 시간을 다루는 도구들을 불러온다
...
def _load_symbol_data_for_bc(sym: str, timeframe: str, start_str: str,
    ...
    """
    print(f"\n  [{sym}] 데이터 로드 중 ({timeframe}) ...")  # 지금 어떤 심볼 데이터를 불러오는지 화면에 출력한다
    ...
    if df_raw is None or df_raw.empty:
        print(f"  [{sym}] ERROR: empty dataframe — skip")  # 데이터가 없다는 경고를 출력한다
        return None, None  # 아무것도 없다는 뜻으로 None 두 개를 돌려준다
    ...
    try:
        ...
        print(f"  [{sym}] CVD fetch failed ({_e}) — fallback")
    ...
    print(f"  [{sym}] LONG={_lv[1]:,}  SHORT={_lv[-1]:,}  HOLD={_lv[0]:,}  "
          f"DISCARD={n_discard:,}  bars={len(df_clean):,}")
    # 피처 배열과 행동 배열을 돌려준다
    return all_features, actions
...
class BalancedBatchSampler:
    ...
    def __init__(self, labels: np.ndarray, batch_size: int = 128):
        ...
        print(
            f"  [BC Sampler] HOLD={len(self.idx_hold):,}  "
            f"LONG={len(self.idx_long):,}  SHORT={len(self.idx_short):,}  "
    ...
    def __iter__(self):
        ...
        return iter(batch)  # 섞인 배치를 반복자로 돌려준다

    # 에폭(전체 학습 한 바퀴)당 배치가 몇 개인지 계산하는 함수
    def n_batches(self, n_epochs_worth: int = 1) -> int:
        """에폭당 배치 수 (전체 인덱스 기준)."""
        # 전체 유효 샘플 수를 구한다
...
def build_batch(indices: np.ndarray, features: np.ndarray,
    ...
    n_feat = features.shape[1]  # 피처(특징값) 개수를 가져온다
    ...
    for idx in indices:
        ...
        if start < 0:
            pad = np.zeros((-start, n_feat), dtype=np.float32)  # 부족한 만큼 0으로 채운 배열을 만든다
            win = np.vstack([pad, features[:idx + 1]])  # 패딩과 실제 데이터를 위아래로 붙인다
        else:
            win = features[start: idx + 1]  # 시작부터 현재 위치까지 데이터를 잘라낸다

        x_list.append(win)          # 입력 창(윈도우)을 리스트에 추가한다
        y_list.append(int(actions[idx]))  # 해당 위치의 정답 행동을 정수로 변환해 추가한다
    ...
    return x, y  # 입력 텐서와 정답 텐서를 함께 돌려준다
...
def evaluate_bc(agent, features: np.ndarray, actions: np.ndarray,
                device: torch.device, batch_size: int = 256) -> dict:
    """검증셋 전체에 대한 정확도 + 클래스별 정밀도 계산."""
    agent.eval()  # 모델을 평가(검증) 모드로 전환한다 (드롭아웃 등 비활성화)
    n = len(features)  # 전체 피처 데이터 개수를 가져온다
    ...
    return {
        ...
        "n_samples":  len(labels_arr),     # 검증에 사용된 샘플 수
        "dist_pred":  {
...
def train_one_epoch(agent, sampler: BalancedBatchSampler,
    ...
    agent.train()  # 모델을 학습 모드로 전환한다 (드롭아웃 등 활성화)
    ...
    _loss_fn = RegretWeightedBCLoss(
        tp_mult=tp_mult,           # 이익 실현 배수 (손실 가중에 사용)
        sl_mult=sl_mult,           # 손절 배수 (손실 가중에 사용)
    ...
    for _ in range(n_batches):
        ...
        loss.backward()  # 손실로부터 역전파(backprop)하여 기울기를 계산한다
        ...
        if isinstance(agent.optimizer, _DQNG):
            ...
            agent.optimizer.step()  # 일반 옵티마이저 스텝 실행

        # 기울기 계산 없이 정확도와 손실을 누적한다
        with torch.no_grad():
            preds = logits.argmax(dim=-1)  # 예측 클래스를 선택한다
            total_correct += (preds == y).sum().item()  # 맞게 예측한 수를 더한다
            total_samples += len(y)                     # 처리한 샘플 수를 더한다
            total_loss    += loss.item()                # 손실값을 더한다
...
def main():
    ...
    parser.add_argument("--days",   type=int, default=1461)  # 4년 = 2022 약세장 포함
    ...
    parser.add_argument("--label-smoothing", type=float, default=LABEL_SMOOTH)
    ...
    parser.add_argument("--parity-weight",   type=float, default=0.005,
                        help="Parity Regularizer 가중치 (BC에서는 극히 약하게 0.005 — RL의 act_sym_loss가 HOLD 균형 주담당)")
    # 후회(Regret) 가중치 옵션: 0이면 비활성, 1이면 기회비용 1배 적용
    parser.add_argument("--regret-weight",   type=float, default=REGRET_W,
    ...
    parser.add_argument("--feat-ver", type=int, default=4, choices=[4, 5],
                        help="Feature version: 4=V4 28-dim (default), 5=V5 54-dim (Frenet)")
    # 재현성을 위한 랜덤 시드 옵션 (미지정 시 매번 다른 결과)
    parser.add_argument("--seed", type=int, default=None,
                        help="재현성을 위한 랜덤 시드 (미지정 시 랜덤)")
    args = parser.parse_args()  # 입력된 인자들을 파싱(분석)해서 args 객체에 저장한다

    # ── 시드 고정 ──────────────────────────────────────────────────────────────
    # 시드(seed)가 지정된 경우에만 랜덤 고정을 실행한다
    if args.seed is not None:
        import random  # 파이썬 기본 랜덤 함수를 불러온다
        ...
        print(f"  [Seed] 랜덤 시드 고정: {args.seed} (재현 가능)")  # 시드 고정 완료 메시지를 출력한다

    os.makedirs(args.checkpoint_dir, exist_ok=True)  # 체크포인트 저장 폴더가 없으면 만든다
    device = torch.device(args.device)  # 학습에 사용할 장치(GPU 또는 CPU) 객체를 만든다

    # 학습 시작 안내 메시지를 화면에 출력한다
    print("═" * 72)
    print("  Phase 0: Behavior Cloning Pre-training")
    print("  DeepMind AlphaGo SL Policy Network 단계")
    print("  목표: val accuracy > 33% (random baseline ≈33%, BEP=20%)")
    print("═" * 72)
    ...
    print(f"\n  [Data] Symbols={_symbol_list}  TF={args.timeframe}  [{_days_info}]")
    ...
    for _sym in _symbol_list:
        ...
        # 데이터가 없으면 이 심볼을 건너뛴다
        if _feats is None:
            continue  # 다음 심볼로 넘어간다
        _N     = len(_feats)                        # 전체 샘플 수를 가져온다
        _split = int(_N * (1 - args.val_ratio))     # 학습/검증 분리 인덱스를 계산한다
        all_train_feats.append(_feats[:_split])     # 앞부분을 학습용 피처에 추가한다
        all_train_acts.append(_acts[:_split])       # 앞부분을 학습용 행동에 추가한다
        all_val_feats.append(_feats[_split:])       # 뒷부분을 검증용 피처에 추가한다
        all_val_acts.append(_acts[_split:])         # 뒷부분을 검증용 행동에 추가한다

    # 유효한 학습 데이터가 하나도 없으면 오류 메시지를 출력하고 함수를 종료한다
    if not all_train_feats:
    ...
    from sklearn.preprocessing import StandardScaler as _SS  # 표준화 스케일러를 불러온다
    ...
    import pickle  # 파이썬 객체를 파일로 저장하는 라이브러리를 불러온다
    # 스케일러 객체를 파일로 저장한다 (나중에 추론 시 같은 스케일러 사용)
    with open(_scaler_path, "wb") as _f:
        pickle.dump(_scaler, _f)  # 스케일러를 이진 파일로 저장한다
    print(f"  [Scaler] StandardScaler 적용 완료 (fit=train) → {_scaler_path}")  # 저장 완료 메시지 출력
    ...
    config = AgentConfig(
        ...
        feature_dim=train_feat.shape[1],  # 피처 차원 수 (V4=28, V5=54)
        checkpoint_dir=args.checkpoint_dir,  # 체크포인트 저장 폴더
    ...
    print(f"  [Agent] feature_dim={config.feature_dim}  device={device}")
    ...
    if _decomposer.use_lda:
        ...
        _train_labels_lda = np.where(train_act == 2, -1, train_act)  # 2→-1 변환, {0=HOLD,1=LONG,-1=SHORT}
    ...
    from src.models.qng_optimizer import DiagonalQNGOptimizer as _DQNG  # 양자 자연 기울기 옵티마이저 불러오기
    ...
    best_ckpt_path  = os.path.join(args.checkpoint_dir, "agent_bc_pretrained.pt")  # 최고 모델 저장 경로
    ...
    print(f"\n  {'Epoch':>5}  {'Loss':>8}  {'TrAcc':>7}  "
          f"{'BalAcc':>7}  {'H%':>5}  {'L%':>5}  {'S%':>5}  {'LR':>8}  Info")
    print("  " + "─" * 68)  # 구분선을 출력한다
    print("  (BalAcc = (HOLD+LONG+SHORT acc)/3 — 분포 편향 제거, random=33.3%)")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        try:
            viz_path = viz.plot_bc_epoch(
                epoch=epoch, n_epochs=args.epochs,  # 현재 에폭과 전체 에폭 수
                train_info=train_info, val_info=val_info,  # 학습/검증 통계
                lr=current_lr, dp=dp,  # 현재 학습률과 예측 분포
            )
            print(f"         [viz] → {viz_path}")  # 저장된 그래프 파일 경로를 출력한다
        ...
        if no_improve >= args.patience:
            print(f"\n  [EarlyStop] {args.patience} epochs without improvement → stopping.")  # 조기 종료 메시지 출력
            break  # 학습 루프를 빠져나간다

    elapsed = time.time() - t0  # 전체 학습에 걸린 시간을 계산한다
    # 학습 완료 요약 정보를 화면에 출력한다
    print("\n" + "═" * 72)
    print(f"  BC Pre-training 완료")
    print(f"  Best BalAcc       : {best_val_acc:.2%}  (Balanced Accuracy, random=33.3%)")
    ...
    print(f"  Elapsed           : {elapsed/60:.1f}min")  # 경과 시간을 분 단위로 출력한다
    print(f"  Checkpoint        : {best_ckpt_path}")  # 저장된 체크포인트 파일 경로를 출력한다
    ...
    if best_val_acc > 0.40:
        print("  ✅ 40%+ — 강한 균형 신호. LONG/SHORT 모두 학습됨. RL 수렴 기대.")  # 40% 이상: 매우 좋음
    elif best_val_acc > 0.36:
        print("  ✅ 36%+ — 유효 신호. 3개 클래스 모두 random 이상 학습됨. RL 진행 가능.")  # 36~40%: 충분히 좋음
    elif best_val_acc > 0.33:
        print("  ⚠ 33~36% — 약한 신호. 일부 클래스만 학습됨. RL 수렴 불확실.")  # 33~36%: 약한 신호
    else:
        print("  ❌ <33% — 랜덤 baseline(33.3%)과 동일 또는 이하.")  # 33% 이하: 학습 실패
        print("     → 시드 변경 또는 피처 엔지니어링 재검토 필요.")  # 다음 조치 안내

    # 다음 단계로 실행할 RL 학습 명령어를 화면에 출력한다
    print("\n  [다음 단계]")
    print(f"  python scripts/train_quantum_v2.py --symbol {args.symbol} "
          f"--timeframe {args.timeframe} \\")
    print(f"    --days {args.days} --rolling-window 360 --n-folds 5 \\")
    print(f"    --pretrain-ckpt {best_ckpt_path}")  # 저장된 BC 체크포인트를 RL 학습의 시작점으로 사용
    print("═" * 72)  # 마무리 구분선을 출력한다
