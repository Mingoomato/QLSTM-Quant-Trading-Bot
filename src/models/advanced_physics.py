...
from __future__ import annotations  # 파이썬의 타입 힌트를 최신 방식으로 쓸 수 있게 한다

import math  # 수학 상수(π 등)와 함수를 쓰기 위한 도구
from typing import Optional, Tuple  # 변수 종류를 명확히 표시하기 위한 도구

import torch  # 딥러닝 프레임워크 (PyTorch)
import torch.nn as nn  # 신경망 레이어들을 쓰기 위한 도구
import torch.nn.functional as F  # 활성화 함수 등 자주 쓰는 연산 모음
...
class HurstEstimator(nn.Module):
    ...
    @staticmethod
    def _rs_single(x: torch.Tensor) -> torch.Tensor:
        """R/S statistic for a 1-D tensor x — 1차원 수익률 시계열의 R/S 통계를 계산한다."""
        n = x.shape[0]  # 시계열 길이
        if n < 4:
            return torch.tensor(0.5, device=x.device, dtype=x.dtype)  # 데이터가 너무 적으면 0.5(무작위) 반환
    ...
    def _hurst_single(self, x: torch.Tensor) -> torch.Tensor:
        """
        ...
        n = x.shape[0]  # 시계열 길이
        if n < 8:
            return torch.tensor(0.5, device=x.device, dtype=x.dtype)  # 너무 짧으면 0.5 반환
        ...
        for k in range(self.n_scales):
            ...
            if s > n // 2:
                break  # 스케일이 시계열의 절반보다 크면 중단
            scales.append(s)  # 유효한 스케일만 추가

        if len(scales) < 2:
            return torch.tensor(0.5, device=x.device, dtype=x.dtype)  # 스케일이 2개 미만이면 계산 불가
        ...
        for s in scales:
            ...
            n_chunks = n // s  # 구간 개수
            if n_chunks < 1:
                continue  # 구간이 없으면 건너뛴다
            rs_vals = []
            for c in range(n_chunks):
                chunk = x[c * s: (c + 1) * s]  # c번째 구간 데이터 추출
                rs_vals.append(self._rs_single(chunk))  # 구간의 R/S 계산
            rs_mean = torch.stack(rs_vals).mean()  # 구간 평균 R/S
            log_ns.append(math.log(s))             # log(스케일) 추가
            log_rs.append(torch.log(rs_mean.clamp(min=1e-8)))  # log(R/S) 추가

        if len(log_ns) < 2:
            return torch.tensor(0.5, device=x.device, dtype=x.dtype)  # 데이터 부족
    ...
    def forward(self, log_returns: torch.Tensor) -> torch.Tensor:
        """
        ...
        Returns:
            hurst: [B] — H ∈ [0.05, 0.95]
        """
        B = log_returns.shape[0]  # 배치 크기
        results = [self._hurst_single(log_returns[b]) for b in range(B)]  # 각 샘플마다 허스트 지수 계산
        return torch.stack(results)                   # [B] 배치 전체 허스트 지수를 쌓아서 반환

    @staticmethod
    def optimal_lookback(H: torch.Tensor, T_base: int = 30) -> torch.Tensor:
        """
        ...
        T_opt = (T_base / sensitivity).ceil().long().clamp(4, T_base * 4)  # 최적 기간 계산 후 4~120 범위로 제한
        return T_opt  # 최적 관찰 기간 반환
...
class LindbladDecoherence(nn.Module):
    """
    ...
    def __init__(
        ...
        n_lindblad: int = 2,  # 점프 연산자 개수 (레짐 전환 경로 수)
        dt: float = 1.0,      # 린드블라드 시간 스텝 크기
    ) -> None:
        super().__init__()  # 부모 클래스 초기화
        self.n_qubits = n_qubits    # 큐비트 수 저장
        self.n_lindblad = n_lindblad  # 점프 연산자 수 저장
        self.dt = dt                  # 시간 스텝 저장
        ...
        self.L_re = nn.Parameter(torch.randn(n_lindblad, dim, dim) * 0.1)  # 실수부
        self.L_im = nn.Parameter(torch.randn(n_lindblad, dim, dim) * 0.1)  # 허수부
        ...
        self.state_encoder = nn.Sequential(
            nn.Linear(n_qubits, dim * dim),  # 큐비트 수 → dim² 차원으로 선형 변환
            nn.Tanh(),                        # tanh 활성화 (-1~1 범위로 정규화)
        )

    @property
    def gamma(self) -> torch.Tensor:
        return torch.exp(self.log_gamma).clamp(max=10.0)  # 결깨짐 속도: log_gamma를 exp로 변환 (최대 10으로 제한)
    ...
    def _encode_density_matrix(self, expvals: torch.Tensor) -> torch.Tensor:
        ...
        rho = rho / tr.clamp(min=1e-8).view(B, 1, 1)         # 대각합으로 나눠서 정규화

        return rho  # 유효한 밀도 행렬 반환

    def _lindblad_step(self, rho: torch.Tensor) -> torch.Tensor:
        """
        ...
        """
        B, dim, _ = rho.shape  # 배치 크기, 힐베르트 공간 차원
        drho = torch.zeros_like(rho)  # 밀도 행렬 변화량 초기화

        for k in range(self.n_lindblad):
        ...
        rho_new = rho_new / tr.clamp(min=1e-8).view(B, 1, 1)  # 정규화

        return rho_new  # 진화된 밀도 행렬 반환
    ...
    def forward(
        ...
        off_diag_mask = (1.0 - eye).unsqueeze(0)  # 비대각 위치만 1인 마스크
        ...
        return purity, coherence, regime_prob  # 순수도, 결맞음, 레짐 변화 확률 반환
...
class MINEEstimator(nn.Module):
    ...
    def __init__(
        ...
        self.T_net = nn.Sequential(
            nn.Linear(x_dim + y_dim, hidden),  # 특징+레이블을 hidden 차원으로 변환
            nn.ELU(),                           # ELU 활성화 함수 (부드러운 음수 허용)
            nn.Linear(hidden, hidden),          # 두 번째 은닉층
            nn.ELU(),                           # ELU 활성화
            nn.Linear(hidden, 1),               # 스칼라 출력
        )

        # EMA of denominator for variance reduction (DV representation)
        # 분산 감소를 위한 분모의 지수이동평균 (학습 안정화)
        self.register_buffer("ema_denom", torch.ones(1))  # 초기값 1
    ...
    def forward(
        ...
        ema_d = self.ema_denom.detach()       # 현재 EMA 분모 (기울기 차단)
        # MINE-f lower bound: E[T_joint] - (e^T_marginal / ema_denom - log(ema_denom) - 1)
        mi_lb = T_joint.mean() - (
            exp_T_marg / ema_d - torch.log(ema_d) - 1.0
        ...
        return mi_lb, T_joint  # 상호 정보량 하한과 결합 T 통계량 반환
...
class MPSLayer(nn.Module):
    ...
    def __init__(
        ...
        self.tensors = nn.ParameterList()  # 각 사이트의 텐서를 담는 목록
        for i in range(n_sites):
            chi_l = 1 if i == 0 else chi          # 첫 사이트는 왼쪽 본드 크기 1
            chi_r = 1 if i == n_sites - 1 else chi  # 마지막 사이트는 오른쪽 본드 크기 1
            A = nn.Parameter(torch.randn(chi_l, d_phys, chi_r) * 0.1)  # 작은 값으로 초기화
            self.tensors.append(A)  # 텐서 목록에 추가

        # Feature embedding: [B, in_features] → [B, n_sites * d_phys]
        # 입력 특징을 각 사이트의 물리 차원 값으로 변환하는 네트워크
        self.embed = nn.Sequential(
            nn.Linear(in_features, n_sites * d_phys),  # 선형 변환
            nn.Tanh(),                                   # tanh 활성화
        )

        # Output projection from contracted result — 수축 결과를 출력 차원으로 변환
        self.proj = nn.Linear(d_phys, out_features)  # 물리 차원 → 출력 차원

        # LayerNorm for stability — 학습 안정화를 위한 레이어 정규화
        self.norm = nn.LayerNorm(out_features)

    def _contract_mps(self, site_inputs: torch.Tensor) -> torch.Tensor:
        """
        ...
        for k in range(1, self.n_sites - 1):
            ...
            chi_l, d, chi_r = Ak.shape                 # 텐서 형태 분리
        ...
        return out.squeeze(1).matmul(
            torch.ones(A_last.shape[0], self.d_phys, device=out.device)
            / (A_last.shape[0] + 1e-8)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return self.norm(out)  # 레이어 정규화 후 반환
...
class OptimalStoppingBoundary(nn.Module):
    """
    ...
    def __init__(
        ...
        self.value_net = nn.Sequential(
            nn.Linear(state_dim, hidden),  # 상태 → 은닉층
            nn.Tanh(),                      # tanh 활성화
            nn.Linear(hidden, hidden),      # 은닉층 → 은닉층
            nn.Tanh(),                      # tanh 활성화
            nn.Linear(hidden, 1),           # 은닉층 → 스칼라 값
        )

        # Adaptive TP multiplier: α_TP(s) ∈ [1.0, 4.0] — 상태에 따른 가변 익절가 배수 네트워크
        self.tp_scale_net = nn.Sequential(
            nn.Linear(state_dim, hidden // 2),  # 상태 → 작은 은닉층
            nn.ReLU(),                           # ReLU 활성화
            nn.Linear(hidden // 2, 1),           # → 스칼라
            nn.Sigmoid(),                        # sigmoid로 0~1 범위
        )

        # Adaptive SL multiplier: α_SL(s) ∈ [0.5, 2.0] — 상태에 따른 가변 손절가 배수 네트워크
        self.sl_scale_net = nn.Sequential(
            nn.Linear(state_dim, hidden // 2),  # 상태 → 작은 은닉층
            nn.ReLU(),                           # ReLU 활성화
            nn.Linear(hidden // 2, 1),           # → 스칼라
            nn.Sigmoid(),                        # sigmoid로 0~1 범위
    ...
    def forward(
        ...
        return continuation_value, tp_mult, sl_mult  # 포지션 유지 가치, TP 배수, SL 배수 반환
    ...
    def stopping_loss(
        ...
        return F.mse_loss(V_curr, target.detach())  # 현재 가치 추정과 목표값의 MSE 손실
...
class FokkerPlanckRegularizer(nn.Module):
    ...
    def forward(
        ...
        B, T, D = state.shape  # 배치 크기, 시간 길이, 특징 차원
        ...
        sigma_floor = observed_returns.std(unbiased=True).clamp(min=1e-6).detach()  # 실제 수익률의 표준편차 (하한값)
        ...
        if squeeze:
            return fp_loss, mu.squeeze(1), sigma.squeeze(1)  # 2D 입력이었으면 다시 1D로 줄여서 반환

        return fp_loss, mu, sigma  # 포커-플랑크 손실, 드리프트, 확산 반환
...
class WassersteinDROLoss(nn.Module):
    ...
    def __init__(
        ...
        self.log_lambda = nn.Parameter(torch.tensor(math.log(lambda_)))

    @property
    def effective_lambda(self) -> torch.Tensor:
        return torch.exp(self.log_lambda).clamp(max=10.0)  # 실효 λ: exp(log_lambda), 최대 10으로 제한
    ...
    def forward(
        ...
        **loss_kwargs,                 # 손실 함수에 전달할 추가 인자
        ...
        x_adv = x.detach().requires_grad_(True)  # 입력에 대한 기울기를 계산하기 위해 grad 활성화
        ...
        grad_norm = grads.view(x.shape[0], -1).norm(dim=-1).mean()  # 각 샘플의 기울기 크기 평균
...
class PlattCalibrator(nn.Module):
    ...
    def temperature(self) -> torch.Tensor:
        ...
        return torch.exp(self.log_temp).clamp(min=0.1, max=10.0)  # 0.1~10.0 범위로 제한
    ...
    def calibrate(
        ...
        if logits.dim() == 3:
            # [B, T, A] 3차원 입력
            cal_logits = logits / T + b.view(1, 1, -1)  # 온도로 나누고 편향 더하기
        else:
            # [B, A] 2차원 입력
            cal_logits = logits / T + b.view(1, -1)     # 온도로 나누고 편향 더하기

        return F.softmax(cal_logits, dim=-1)  # softmax로 확률로 변환 (합=1)

    @torch.no_grad()  # 기울기 계산 없이 실행 (검증용)
    def compute_ece(
        self,
    ...
    def calibration_nll_loss(
        ...
        log_probs = torch.log(cal_probs.clamp(min=1e-8))  # 로그 확률 (0이 되지 않게 최소값 보장)
        nll = F.nll_loss(log_probs, labels)             # NLL 손실 계산
        return nll  # 보정 NLL 손실 반환
...
class EntropyProductionEstimator:
    ...
    def update(self, action: int) -> None:
        ...
        self._history.append(int(action))  # 행동을 정수로 변환해서 이력에 추가
    ...
    def compute(self) -> float:
        ...
        """
        import math as _math   # 수학 함수 (log 등)
        import numpy as _np    # 수치 계산 도구

        n = len(self._history)  # 이력 길이
        if n < 10:
    ...
    def allows_entry(self) -> bool:
        ...
        if self.threshold <= 0.0:
            return True  # 임계값이 0 이하면 항상 허용 (진단 전용 모드)
        return self._last_s_dot >= self.threshold  # Ṡ가 임계값 이상이면 True

    @property
    def s_dot(self) -> float:
        """Last computed entropy production rate — 마지막으로 계산한 엔트로피 생산율."""
        return self._last_s_dot

    def __repr__(self) -> str:
        return (
            f"EntropyProductionEstimator("
            f"n_states={self.n_states}, window={self.window}, "
            f"threshold={self.threshold}, s_dot={self._last_s_dot:.5f})"
...
def build_advanced_physics(
    ...
    modules = {
        "hurst": HurstEstimator(n_scales=5),  # 허스트 지수 추정기 (5개 스케일)
        "lindblad": LindbladDecoherence(n_qubits=n_qubits, n_lindblad=2),  # 린드블라드 결깨짐 (2개 점프 연산자)
        ...
        "platt": PlattCalibrator(n_classes=n_actions, n_bins=10),  # 플랫 보정기 (10개 구간)
    }

    for name, module in modules.items():
        modules[name] = module.to(device)  # 모든 모듈을 지정 디바이스로 이동한다

    return modules  # 모듈 딕셔너리 반환
...
if __name__ == "__main__":
    ...
    print("=" * 70)  # 구분선
    print("  advanced_physics.py — Self-Test")  # 테스트 제목
    print("=" * 70)

    # ── 1. HurstEstimator ─────────────────────────────────────────────────
    print("\n[1] HurstEstimator")  # 허스트 추정기 테스트
    hurst_est = HurstEstimator(n_scales=4)  # 4개 스케일로 추정기 생성
    lr = torch.randn(B, T) * 0.01  # 임의 로그 수익률 생성 (작은 값)
    H = hurst_est(lr)  # 허스트 지수 계산
    T_opt = HurstEstimator.optimal_lookback(H, T_base=30)  # 최적 관찰 기간 계산
    print(f"  H = {H.tolist()}")  # 허스트 지수 출력
    print(f"  T_opt = {T_opt.tolist()}")  # 최적 기간 출력
    assert H.shape == (B,)  # 형태 검증
    assert ((H >= 0.05) & (H <= 0.95)).all(), "H out of range"  # 범위 검증
    print("  ✓ Hurst in [0.05, 0.95]")  # 통과

    # ── 2. LindbladDecoherence ────────────────────────────────────────────
    print("\n[2] LindbladDecoherence")  # 린드블라드 결깨짐 테스트
    lindblad = LindbladDecoherence(n_qubits=2, n_lindblad=2)  # 2큐비트, 2점프 연산자
    expvals = torch.randn(B, K) * 0.5  # 임의 측정값 생성
    purity, coherence, regime_prob = lindblad(expvals)  # 순수도, 결맞음, 레짐 확률 계산
    print(f"  purity     = {purity.tolist()}")  # 순수도 출력
    print(f"  coherence  = {coherence.tolist()}")  # 결맞음 출력
    print(f"  regime_prob= {regime_prob.tolist()}")  # 레짐 확률 출력
    assert purity.shape == (B,)  # 형태 검증
    print("  ✓ Lindblad shapes OK")  # 통과

    # ── 3. MINEEstimator ──────────────────────────────────────────────────
    print("\n[3] MINEEstimator")  # MINE 추정기 테스트
    mine = MINEEstimator(x_dim=K, y_dim=3, hidden=32)  # 특징 차원 K, 레이블 3, 은닉 32
    x_feat = torch.randn(B, K)  # 임의 특징 생성
    y_label = F.one_hot(torch.randint(0, 3, (B,)), num_classes=3).float()  # 임의 원-핫 레이블 생성
    ...
    print("  ✓ MINE forward OK")  # 통과

    # ── 4. MPSLayer ───────────────────────────────────────────────────────
    print("\n[4] MPSLayer")  # MPS 레이어 테스트
    mps = MPSLayer(n_sites=K, d_phys=2, chi=4, in_features=K, out_features=K)  # MPS 레이어 생성
    x_in = torch.randn(B, K)  # 임의 입력 생성
    out = mps(x_in)  # MPS 순전파 실행
    print(f"  MPS output shape: {tuple(out.shape)}")  # 출력 형태 확인
    assert out.shape == (B, K)  # 형태 검증
    # Gradient flow — 기울기가 제대로 흐르는지 확인
    loss_mps = out.sum()  # 모든 출력 합산
    loss_mps.backward()  # 역전파 실행
    assert mps.tensors[0].grad is not None  # 첫 번째 텐서에 기울기가 있는지 확인
    print("  ✓ MPS gradient flow OK")  # 통과

    # ── 5. OptimalStoppingBoundary ────────────────────────────────────────
    print("\n[5] OptimalStoppingBoundary")  # 최적 정지 경계 테스트
    stop_bound = OptimalStoppingBoundary(state_dim=K + 2, hidden=16)  # K+2 차원 상태 입력
    state = torch.randn(B, K + 2)  # 임의 상태 생성
    V, tp_mult, sl_mult = stop_bound(state)  # 포지션 유지 가치, TP/SL 배수 계산
    print(f"  V (continuation): {V.tolist()}")  # 유지 가치 출력
    ...
    print("  ✓ TP/SL multiplier ranges OK")  # 통과

    # ── 6. FokkerPlanckRegularizer ────────────────────────────────────────
    print("\n[6] FokkerPlanckRegularizer")  # 포커-플랑크 정규화기 테스트
    fp_reg = FokkerPlanckRegularizer(state_dim=K, hidden=16)  # 정규화기 생성
    states_t = torch.randn(B, T, K)   # 임의 상태 시계열 생성
    returns_t = torch.randn(B, T) * 0.01  # 임의 수익률 시계열 (작은 값)
    fp_loss, mu, sigma = fp_reg(states_t, returns_t)  # FP 손실, 드리프트, 확산 계산
    print(f"  FP loss = {fp_loss.item():.4f}")  # FP 손실 출력
    print(f"  mu range:   [{mu.min().item():.4f}, {mu.max().item():.4f}]")  # 드리프트 범위
    print(f"  sigma range:[{sigma.min().item():.4f}, {sigma.max().item():.4f}]")  # 확산 범위
    assert (sigma > 0).all()  # 확산이 항상 양수인지 검증
    print("  ✓ FokkerPlanck loss OK, sigma > 0")  # 통과

    # ── 7. PlattCalibrator ────────────────────────────────────────────────
    print("\n[7] PlattCalibrator")  # 플랫 보정기 테스트
    platt = PlattCalibrator(n_classes=3, n_bins=5)  # 3 클래스, 5개 구간 보정기 생성
    logits_test = torch.randn(B, 3)  # 임의 로짓 생성
    labels_test = torch.randint(0, 3, (B,))  # 임의 정답 레이블 생성
    cal_probs = platt(logits_test)  # 보정된 확률 계산
    ece = platt.compute_ece(cal_probs, labels_test)  # ECE 계산
    nll = platt.calibration_nll_loss(logits_test, labels_test)  # 보정 NLL 계산
    print(f"  Calibrated probs: {cal_probs.detach().numpy().round(3).tolist()}")  # 보정 확률 출력
    print(f"  ECE = {ece:.4f}  (0 = perfect calibration)")  # ECE 출력 (0에 가까울수록 좋음)
    print(f"  Calibration NLL = {nll.item():.4f}")  # 보정 NLL 출력
    assert cal_probs.shape == (B, 3)  # 형태 검증
    assert abs(cal_probs.sum(dim=-1) - 1.0).max().item() < 1e-4  # 확률 합=1 검증
    print("  ✓ Platt calibration OK")  # 통과

    # ── 8. build_advanced_physics factory ─────────────────────────────────
    print("\n[8] build_advanced_physics (factory)")  # 팩토리 함수 테스트
    phys = build_advanced_physics(n_qubits=2, state_dim=K, n_actions=3)  # 모든 모듈 생성
    print(f"  Built modules: {list(phys.keys())}")  # 생성된 모듈 목록 출력
    print("  ✓ Factory OK")  # 통과

    print("\n" + "=" * 70)
    print("  ✓ All advanced_physics.py tests passed!")  # 모든 테스트 통과
    print("=" * 70)
