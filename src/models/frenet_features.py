...
def _frenet_2d(pts: np.ndarray):
    ...
    s_prev = float(np.linalg.norm(d_prev)) + 1e-8
    # 현재 구간의 길이
    s_curr = float(np.linalg.norm(d_curr)) + 1e-8
    ...
    kappa_arc = float(np.linalg.norm(dT)) / (ds + 1e-8)
    ...
    return T_curr.astype(np.float32), kappa, turn_sign


# ── Frenet frame: 3D (B removed — B = T×N is nonlinearly determined) ─────────

def _frenet_3d(pts: np.ndarray):
    """
    ...
    # 기본 주법선 벡터: y방향(위)
    N0 = np.array([0., 1., 0.], dtype=np.float32)

    # 점이 3개보다 적으면 기본값(접선, 곡률=0, 비틀림=0, 주법선)을 반환한다
    if len(pts) < 3:
        return T0, 0.0, 0.0, N0
    ...
    if len(pts) >= 4:
        ...
        d_pp   = p0 - p3
        # 그 구간의 길이
        s_pp   = float(np.linalg.norm(d_pp)) + 1e-8
        # t-3에서 t-2까지의 단위 접선 벡터
        T_pp   = d_pp / s_pp
        # 더 이전 구간의 접선 변화량
        dT_pp  = T_prev - T_pp
        # 더 이전 접선 변화량의 크기
        norm_dT_pp = float(np.linalg.norm(dT_pp))
        ...
        B_prev = np.cross(T_prev, N_prev).astype(np.float32)
        # 이전 이중법선의 크기
        norm_Bp = float(np.linalg.norm(B_prev))
        # 이전 이중법선을 정규화한다
        if norm_Bp > 1e-7:
            B_prev = B_prev / norm_Bp

        # 현재 이중법선(B) = 현재 접선 × 현재 주법선
        B_curr = np.cross(T_curr, N).astype(np.float32)
        # 현재 이중법선의 크기
        norm_Bc = float(np.linalg.norm(B_curr))
        # 현재 이중법선을 정규화한다
        if norm_Bc > 1e-7:
        ...
        tau_raw = float(-np.dot(dB, N) / (ds2 + 1e-8))
...
def build_frenet_features(df: pd.DataFrame, n_traj: int = 4) -> np.ndarray:
    ...
    if n < n_traj + 20:
        return np.zeros(FRENET_DIM, dtype=np.float32)
    ...
    ema12_dev = np.log(closes / np.maximum(ema12, eps))
    # MACD 값: 단기 이동평균과 장기 이동평균의 로그 비율 (추세의 강도)
    macd_val  = np.log(ema12  / np.maximum(ema26, eps))
    ...
    cvd_pts = []
    for lag in range(n_traj - 1, -1, -1):
        # lag만큼 과거의 인덱스를 계산한다
        idx = n - 1 - lag
        # 인덱스가 0보다 작으면 데이터가 없으므로 기본값(0, 0)을 넣는다
        if idx < 1:
            cvd_pts.append([0.0, 0.0])
            continue
        # 해당 시점의 델타를 z-score로 변환하고 [-π, π]로 제한한다
        delta_z = float(np.clip(deltas[idx] / d_std, -np.pi, np.pi))
        # 최근 최대 20봉의 누적 거래량 흐름(CVD 추세)을 계산한다
        w       = min(20, idx + 1)
        # 최근 w봉의 델타 합계
        cum20   = float(deltas[idx + 1 - w: idx + 1].sum())
        # CVD 추세를 z-score로 변환하고 [-π, π]로 제한한다
        trend_z = float(np.clip(cum20 / (d_std * (w ** 0.5) + eps), -np.pi, np.pi))
        cvd_pts.append([delta_z, trend_z])

    # CVD 위상 좌표 배열로 변환한다
    cvd_traj = np.array(cvd_pts, dtype=float)
    ...
    for lag in range(n_traj - 1, -1, -1):
        ...
        mom_pts.append([m3, m10, m20])

    # 모멘텀 좌표 배열로 변환한다
    mom_traj = np.array(mom_pts, dtype=float)
    ...
    for lag in range(n_traj - 1, -1, -1):
        ...
        pv_pts.append([
            ...
            float(np.clip(log_rets[idx],  -np.pi, np.pi)),
            # 거래량 비율을 [-π, π] 범위로 제한한다
            float(np.clip(vol_ratio[idx], -np.pi, np.pi)),
        ])

    # 가격-거래량 위상 좌표 배열로 변환한다
    pv_traj = np.array(pv_pts, dtype=float)
...
assert len(FRENET_FEATURE_NAMES) == FRENET_DIM
...
if __name__ == "__main__":
    ...
    close = 50000.0 + np.cumsum(rng.normal(0, 200, n))
    ...
    df = pd.DataFrame({
        ...
        "low":    np.maximum(close - rng.uniform(0, 400, n), 1.0),
        # 종가
        "close":  close,
        # 거래량: 1백만~1천만 사이의 랜덤값
    ...
    # 특징 벡터의 모양(크기)을 출력한다
    print(f"shape : {feat.shape}   (expected: ({FRENET_DIM},))")
    # 특징값의 최솟값과 최댓값 범위를 출력한다
    print(f"range : [{feat.min():.4f}, {feat.max():.4f}]  (expected: [-pi, pi])")
    # NaN(숫자가 아닌 값)이 있는지 확인한다
    print(f"NaN   : {np.isnan(feat).any()}   (expected: False)")
    print()
    # 각 특징의 이름과 값을 출력한다
    for i, name in enumerate(FRENET_FEATURE_NAMES):
        print(f"  [{i:2d}] {name:<28s} = {feat[i]:+.4f}")

    # 특징 벡터의 모양이 (20,)인지 확인한다
    assert feat.shape == (FRENET_DIM,)
    # NaN이 없는지 확인한다
    assert not np.isnan(feat).any()
    # 무한대 값이 없는지 확인한다
    assert not np.isinf(feat).any()
    # 모든 값이 [-π, π] 범위 안에 있는지 확인한다 (약간의 오차 허용)
    assert np.all(np.abs(feat) <= np.pi + 1e-5)
    print("\n[PASS] frenet_features.py self-test OK")
