...
class AdaptiveTemperatureScaler(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.log_t_base = nn.Parameter(torch.tensor(math.log(t_base)))
        self.log_beta   = nn.Parameter(torch.tensor(math.log(beta)))
        self.t_min = t_min
        self.t_max = t_max

    @property
    def t_base(self) -> torch.Tensor:
        return torch.exp(self.log_t_base)
    ...
    def forward(
        ...
        if logits.dim() == 3:
            ...
            t_scaled = t_temp.view(-1, 1, 1)
        else:
            # [B, A] 케이스: atr 를 [B, 1] 로
            t_scaled = t_temp.view(-1, 1)

        # softmax(z / T_temp)  [마스터플랜 Part 3.3]
        probs = F.softmax(logits / t_scaled, dim=-1)
        return probs, t_temp
...
class PathIntegralLoss(nn.Module):
    ...
    @property
    def effective_eta(self) -> torch.Tensor:
        """실효 η = η_base × leverage × learned_scale"""
        return self.eta * torch.exp(self.log_eta_scale)

    # ─────────────────────────────────────────────────────────────────────
    # γ^t 할인 벡터 생성
    # ─────────────────────────────────────────────────────────────────────

    def _discount_vector(
    ...
    def _terminal_reward(
        ...
        dtype  = realized_pnl.dtype
        B = terminal_state.shape[0]

        R_t = torch.zeros(B, device=device, dtype=dtype)

        # ── E: TP Hit → +r_tp ────────────────────────────────────────────
        tp_mask = (terminal_state == MarketState.TP_HIT)
        R_t = R_t + self.r_tp * tp_mask.to(dtype)

        # ── F: SL Hit → +r_sl (음수) ─────────────────────────────────────
        sl_mask = (terminal_state == MarketState.SL_HIT)
        R_t = R_t + self.r_sl * sl_mask.to(dtype)
        ...
        r_agile = (
            ...
            * torch.sigmoid(5.0 * realized_pnl)
        )
        R_t = R_t + r_agile * sc_mask.to(dtype)
    ...
    def _compute_path_reward(self, path: TradingPath) -> torch.Tensor:
        """
        마스터플랜 핵심 수식 전체 계산:
            J(τ) = Σ_{t=0}^{T} γ^t ΔV_t  -  η · I_{open}  +  R_terminal(S_T)
        ...
        discounted_pnl = (gamma_t.unsqueeze(0) * delta_v * hold_mask).sum(dim=1)
        # [B]
        ...
        R_terminal = self._terminal_reward(
            path.terminal_state, path.realized_pnl
        )                                                    # [B]
    ...
    @staticmethod
    def _entropy_bonus(probs: torch.Tensor) -> torch.Tensor:
        """
        정책 엔트로피 정규화항.
    ...
    def forward(
        ...
        if logits.dim() == 3:
            ...
            log_pi = log_probs_all.gather(
                dim=2, index=action_idx.unsqueeze(2)
            ).squeeze(2)                                     # [B, T]
            ...
            policy_loss = -(adv_broadcast * log_pi * entry_mask).sum(dim=1).mean()
            ...
            log_pi     = log_probs_all.gather(
                dim=1, index=action_idx.unsqueeze(1)
            ).squeeze(1)                                     # [B]
            # Fix C: HOLD(0) 제외 — LONG/SHORT만 policy gradient 기여
            active_mask = (action_idx > 0).float()
            n_active = active_mask.sum().clamp(min=1.0)
            policy_loss = -(advantage * log_pi * active_mask).sum() / n_active
        ...
        loss = reward_loss + policy_loss - self.entropy_reg * entropy
        ...
        info = {
            ...
            "J_min":         J.min().item(),
            "J_max":         J.max().item(),
            "eta_effective": self.effective_eta.item(),
            "t_temp_mean":   t_temp.mean().item(),
            # MarketState 분포
...
class TradingPathBuilder:
    ...
    def from_tensors(
        ...
        entry_prices = prices.gather(
            1, entry_idx.clamp(0, T-1).unsqueeze(1)
        ).squeeze(1).clamp(min=1e-8)                         # [B]

        # ΔV_t = (ΔPrice_t / P_entry) × direction × leverage
        direction_f = directions.to(dtype)
        delta_v = (
            price_diff
            / entry_prices.unsqueeze(1)
            * direction_f.unsqueeze(1)
            * self.leverage
        )                                                    # [B, T]
        ...
        label_to_state = {
            1: MarketState.TP_HIT,
            2: MarketState.TP_HIT,
            3: MarketState.SL_HIT,
        }
        terminal_state = torch.full(
            (B,), fill_value=MarketState.OBSERVE, device=device, dtype=torch.long
        )
        for label_val, state_val in label_to_state.items():
        ...
        dir_short = (directions.long() == -1) # [B] bool
...
class GeneralizedAdvantage(nn.Module):
    """
    ...
    def forward(
        ...
        if hurst is not None:
            H_val  = float(max(0.3, min(0.95, hurst)))
            lam_eff = float(max(0.5, min(0.99, self.lam + 0.04 * (H_val - 0.5))))
        else:
            lam_eff = self.lam

        # Bootstrap value at t=T (zero for done episodes)
        bootstrap = torch.zeros(B, device=device, dtype=rewards.dtype)

        advantages = torch.zeros_like(rewards)           # [B, T]
        last_gae = torch.zeros(B, device=device, dtype=rewards.dtype)
        ...
        for t in reversed(range(T)):
            ...
            if dones is not None:
                ...
                mask = torch.ones(B, device=device, dtype=rewards.dtype)

            # TD residual: δ_t = r_t + γV(s_{t+1}) - V(s_t)
            delta = rewards[:, t] + self.gamma * next_value * mask - values[:, t]

            # GAE: Â_t = δ_t + (γλ) Â_{t+1}  [lam_eff is H-adaptive for GLE]
            last_gae = delta + self.gamma * lam_eff * mask * last_gae
...
def _quantile_huber_loss(
    ...
    rho = (tau - (u < 0).to(tau.dtype)).abs() * huber
    return rho.mean()
...
class IQNCriticHead(nn.Module):
    ...
    def forward(
        ...
        """
        N = n_samples or self.n_quantiles
        B, T, K = c_kt.shape
        ...
        i_vec = torch.arange(1, self.N_EMBED + 1, device=device, dtype=dtype)
        ...
        return quant_vals, cvar, mean_val, tau
...
class AdvancedPathIntegralLoss(nn.Module):
    """
    ...
    def __init__(
        ...
        self.log_eta_scale = nn.Parameter(torch.tensor(0.0))
        ...
        self._use_iqn = use_iqn
        if use_iqn:
            self.critic = IQNCriticHead(
                state_dim=state_dim,
                hidden=64,
                n_quantiles=iqn_quantiles,
                cvar_alpha=cvar_alpha,
            )
        else:
            self.critic = CriticHead(state_dim=state_dim, hidden=64)
    ...
    @staticmethod
    def _quick_hurst(returns_flat: "np.ndarray") -> float:
        """
        ...
        try:
            ...
            S   = x.std() + 1e-8
            rs  = R / S
            H   = _math.log(max(rs, 1e-8)) / _math.log(T)
            return float(_np.clip(H, 0.3, 0.9))
        except Exception:
            return 0.5
    ...
    def forward(
        ...
        B_p, T_p = path.delta_v.shape
        gamma_t = self.gamma ** torch.arange(T_p, device=device, dtype=dtype)
        hold_mask = (
            (path.event_mask == MarketState.HOLD)
            | (path.event_mask == MarketState.LONG)
            | (path.event_mask == MarketState.SHORT)
        ...
        terminal_R = self._terminal_reward(path.terminal_state, path.realized_pnl)
        # Add terminal reward at last timestep of the window
        step_rewards_with_terminal = step_rewards.clone()
        ...
        entry_idx_clamp = torch.zeros(B, device=device, dtype=torch.long)
        step_rewards_with_terminal.scatter_add_(
            1,
            entry_idx_clamp.unsqueeze(1),
            -eta_pen.unsqueeze(1),
        )

        # ── Align T_path vs T_critic (may differ when x-window ≠ price-window) ──
        _T_p = step_rewards_with_terminal.shape[1]
        if _T_c != _T_p:
            _T = min(_T_c, _T_p)
        ...
        if self._use_gle:
            import numpy as _np_gle
            rets_np = step_rewards_with_terminal.detach().cpu().float().numpy().flatten()
            _H = self._quick_hurst(rets_np)
        else:
            _H = None
        ...
        if not is_last_step:
            A_dim = logits.shape[-1]
            _T_act = advantages.shape[1]
            ...
            actor_loss = -(advantages * log_pi * entry_mask).sum(dim=1).mean()
            ...
            adv_scalar = advantages.mean(dim=1)
            ...
            adv_norm = adv_scalar.clone()
            if active_mask_bool.sum() > 1:
                _a_sub = adv_scalar[active_mask_bool]
            ...
            n_active = active_mask.sum().clamp(min=1.0)
            actor_loss = -(adv_norm * log_pi * active_mask).sum() / n_active
            entropy = -(probs * log_probs_all).sum(dim=-1).mean()
        ...
        if self._use_iqn:
            critic_loss = _quantile_huber_loss(
                _iqn_qv, returns.detach(), _iqn_tau
            )
        else:
        ...
        dir_sym_loss = ((mean_p - target) **2).sum()
        ...
        J_mean = step_rewards_with_terminal.sum(dim=1).mean()
        loss = (
        ...
        info = {
            ...
            "fp_loss":       fp_loss.item(),
            "dir_sym_loss":  dir_sym_loss.item(),
            "act_sym_loss":  act_sym_loss.item(),
            "entropy":       entropy.item(),
            "J_mean":        J_mean.item(),
            "J_std":         step_rewards_with_terminal.sum(dim=1).std().item(),
            "eta_effective": self.effective_eta.item(),
            "t_temp_mean":   t_temp.mean().item(),
            ...
            "J_min":         step_rewards_with_terminal.sum(dim=1).min().item(),
            "J_max":         step_rewards_with_terminal.sum(dim=1).max().item(),
            # Per-sample advantage [B] tensor — used by AWBC in train_step.
            # Not a scalar: intentionally stored as tensor for gate computation.
            "adv_per_sample": advantages.mean(dim=1).detach(),
        }
...
class AdvantageWeightedBC(nn.Module):
    """
    ...
    def forward(
        ...
        if advantages is not None:
            ...
            adv_pos = torch.ones_like(active)
        ...
        return self.weight * (gate * ce_per).sum() / gate_sum
...
class RegretWeightedBCLoss(nn.Module):
    """
    ...
    def forward(
        ...
        # Regret 가중 적용
        ce_loss = (regret_weights.detach() * ce_per_sample).mean()
        ...
        return total, {
            "ce":     ce_loss.item(),
            "regret": expected_regret.mean().item(),
            "orth":   orth_loss.item() if isinstance(orth_loss, torch.Tensor) else 0.0,
            "parity": parity_loss.item(),
        }


class QuantumDivineLossV2(nn.Module):
    """
    ...
    def forward(
        ...
        with torch.no_grad():
            soft = torch.full_like(logits, self.smoothing / (self.n_class - 1))
            soft.scatter_(1, labels.unsqueeze(1), 1.0 - self.smoothing)
        ce_loss = -(soft * F.log_softmax(logits, dim=1)).sum(dim=1).mean()
...
if __name__ == "__main__":
    print("=" * 65)
    print("  PathIntegralLoss — Phase 3 자가 검증 (Self-Test)")
    print("  마스터플랜 Part 3.1 / 3.2 / 3.3 수식 반영 확인")
    print("=" * 65)
    ...
    # ── 1. 온도 스케일러 검증 ────────────────────────────────────────────
    print("\n[Test 1] AdaptiveTemperatureScaler")
    ...
    print(f"  저변동성 T_temp: {t_low[0].item():.4f}")
    print(f"  고변동성 T_temp: {t_high[0].item():.4f}")
    assert t_high[0] > t_low[0], "고변동성에서 온도가 더 높아야 함!"
    print(f"  ✓ 고변동성 > 저변동성 온도 검증 통과")

    # ── 2. 경로 보상 J(τ) 검증 ──────────────────────────────────────────
    print("\n[Test 2] _compute_path_reward")
    loss_fn = build_path_integral_loss(gamma=0.99, leverage=25.0)
    ...
    path_sc = TradingPath(
        ...
        realized_pnl=torch.full((B,), 0.03),
        hold_steps=torch.full((B,), T // 2, dtype=torch.long),
        atr_norm=torch.full((B,), 0.01),
        opened=torch.ones(B, dtype=torch.bool),
    )
    J_sc = loss_fn._compute_path_reward(path_sc)
    print(f"  SC 경로 J(τ): {J_sc.mean().item():.4f}  (0.5~0.9 기대)")

    assert J_tp.mean() > J_sl.mean(), "TP > SL 검증 실패!"
    assert J_tp.mean() > J_sc.mean(), "TP > SC 검증 실패!"
    assert J_sc.mean() > J_sl.mean(), "SC > SL 검증 실패!"
    print("  ✓ 보상 크기 순서: TP > SC > SL 검증 통과")

    # γ 할인 효과 검증: 짧은 hold 가 더 유리해야 함
    path_fast = TradingPath(
        delta_v=torch.cat([torch.full((B, 5), 0.01), torch.zeros(B, T-5)], dim=1),
        event_mask=event_mask_tp,
        terminal_state=torch.full((B,), MarketState.TP_HIT, dtype=torch.long),
        realized_pnl=torch.full((B,), 0.05),
        hold_steps=torch.full((B,), 5, dtype=torch.long),
        atr_norm=torch.full((B,), 0.01),
        opened=torch.ones(B, dtype=torch.bool),
    )
    path_slow = TradingPath(
        delta_v=torch.cat([torch.zeros(B, T-5), torch.full((B, 5), 0.01)], dim=1),
        event_mask=event_mask_tp,
        terminal_state=torch.full((B,), MarketState.TP_HIT, dtype=torch.long),
        realized_pnl=torch.full((B,), 0.05),
        hold_steps=torch.full((B,), T, dtype=torch.long),
    ...
    print(f"\n  γ 할인 효과 (동일 수익):")
    print(f"    빠른 수익(t=0~4): J = {J_fast.mean().item():.4f}")
    print(f"    느린 수익(t=25~29): J = {J_slow.mean().item():.4f}")
    assert J_fast.mean() > J_slow.mean(), "γ 할인: 빠른 수익이 더 유리해야 함!"
    print("  ✓ 시간 할인 γ^t 효과 검증 통과 (빠른 수익 > 느린 수익)")

    # ── 3. Forward + backward 검증 ──────────────────────────────────────
    print("\n[Test 3] Forward + backward (그라디언트 흐름)")
    logits_3d = torch.randn(B, T, A, requires_grad=True)
    loss, info = loss_fn(logits_3d, path_tp)
    loss.backward()
    grad_ok = logits_3d.grad is not None
    print(f"  loss = {info['loss']:.4f}")
    print(f"  J_mean = {info['J_mean']:.4f}")
    print(f"  eta_effective = {info['eta_effective']:.6f}")
    print(f"  t_temp_mean = {info['t_temp_mean']:.4f}")
    print(f"  ✓ backward() 그라디언트 흐름: {'OK' if grad_ok else 'FAIL'}")

    # ── 4. TradingPathBuilder 검증 ──────────────────────────────────────
    print("\n[Test 4] TradingPathBuilder (원시 데이터 → TradingPath)")
    builder = TradingPathBuilder(leverage=25.0)
    prices = 50000.0 + torch.cumsum(torch.randn(B, T) * 50, dim=1)
    directions = torch.ones(B)
    entry_idx = torch.zeros(B, dtype=torch.long)
    labels = torch.randint(0, 4, (B,))
    atr = torch.full((B,), 150.0)
    path_built = builder.from_tensors(prices, directions, entry_idx, labels, atr)
    print(f"  delta_v shape: {tuple(path_built.delta_v.shape)}")
    print(f"  event_mask sample: {path_built.event_mask[0, :5].tolist()}")
    print(f"  terminal_state: {path_built.terminal_state.tolist()}")
    J_built = loss_fn._compute_path_reward(path_built)
    print(f"  J(τ) range: [{J_built.min().item():.4f}, {J_built.max().item():.4f}]")
    print("  ✓ TradingPathBuilder 검증 통과")

    print("\n  ✓ 모든 검증 통과! Phase 3 구현 완료.")
    print("=" * 65)
