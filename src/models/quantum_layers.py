...
from __future__ import annotations  # 파이썬의 타입 힌트를 최신 방식으로 사용할 수 있게 한다  # Import annotations from __future__ module

import itertools  # 큐비트 쌍 조합을 만들기 위한 도구  # Import Iterator building blocks
import math  # 원주율(π) 같은 수학 상수를 쓰기 위한 도구  # Import Python standard math library (log, exp, trig)
from typing import List, Optional, Tuple  # 변수의 종류를 명확히 표시하기 위한 도구  # Import List, Optional, Tuple from Type hint annotations

import pennylane as qml  # 양자 컴퓨터 시뮬레이션 도구 (PennyLane)  # Import PennyLane quantum ML module as "qml"
...
N_VQC_LAYERS: int = 1      # VQC(양자 회로)를 몇 층 쌓을지 (많을수록 표현력↑, 속도↓)
...
class IsingCouplingEstimator(nn.Module):  # ★ [IsingCouplingEstimator] 클래스 정의 — 관련 데이터와 기능을 하나로 묶은 설계도
    """
    ...
    def __init__(  # [__init__] 초기화 메서드 — 객체를 만들 때 가장 먼저 실행된다
        ...
        super().__init__()  # 부모 클래스(nn.Module)의 초기화 함수를 먼저 실행한다  # Calls the parent class constructor
        ...
        self.log_alpha = nn.Parameter(  # 학습 중 자동 업데이트되는 파라미터로 등록한다  # Registers tensor as a learnable parameter (tracked by autograd)
            torch.tensor(math.log(alpha_init), dtype=torch.float32)  # 파이썬 데이터를 파이토치 텐서로 변환한다  # Converts Python data to a PyTorch tensor
    ...
    def alpha(self) -> torch.Tensor:  # [alpha] 함수 정의 시작
    ...
    def forward(  # [forward] 신경망의 순방향 계산을 정의한다 (입력 → 출력)
        ...
        """
        if self.use_delta_c and delta_c_kt is not None:  # 조건이 참일 때만 실행한다  # Branch: executes only when condition is True
        ...
        # ── Fix A: T<2 방어 ───────────────────────────────────────────
        if T < 2:  # 조건이 참일 때만 실행한다  # Branch: executes only when condition is True
            # 데이터가 1개뿐이면 상관계수를 계산할 수 없다
            if self._j_last is not None:  # 조건이 참일 때만 실행한다  # Branch: executes only when condition is True
                return self._j_last.expand(B, -1, -1)  # 이전에 계산한 J를 배치 크기에 맞게 확장해서 돌려준다  # Returns a value to the caller
            return torch.zeros(B, K, K, device=feat.device, dtype=feat.dtype)  # 이전 값도 없으면 0 행렬 반환  # Returns a value to the caller
        ...
        eye = torch.eye(K, device=feat.device, dtype=feat.dtype).unsqueeze(0)  # 단위 행렬 (대각선이 1)  # Inserts a new dimension of size 1
        ...
        self._j_last = J.detach().mean(dim=0, keepdim=True)  # 배치 평균을 저장해두어 다음 호출 시 재사용한다  # Detaches tensor from the computation graph

        return J  # 이징 상호작용 행렬을 돌려준다  # Returns a value to the caller
...
class TemporalContextEncoder(nn.Module):  # ★ [TemporalContextEncoder] 클래스 정의 — 관련 데이터와 기능을 하나로 묶은 설계도
    """
    ...
    def __init__(  # [__init__] 초기화 메서드 — 객체를 만들 때 가장 먼저 실행된다
        ...
        self.proj_in = nn.Linear(in_dim, d_model)  # 입력을 Transformer 크기로 변환하는 선형 레이어  # Stores a neural network layer as an attribute of this module
        encoder_layer = nn.TransformerEncoderLayer(  # 어텐션 기반 시퀀스 처리 트랜스포머 레이어를 만든다  # Transformer (multi-head attention) layer
        ...
        self.transformer = nn.TransformerEncoder(encoder_layer, num_layers=n_layers)  # 여러 층의 Transformer 인코더  # Stores a neural network layer as an attribute of this module
    ...
    def forward(self, c_kt: torch.Tensor) -> torch.Tensor:  # [forward] 신경망의 순방향 계산을 정의한다 (입력 → 출력)
        ...
        return c_kt + delta         # 원래 입력에 Transformer가 찾아낸 패턴을 더한다 (잔차 연결)  # Returns a value to the caller
...
def _make_device(n_qubits: int, use_lightning: bool = True) -> qml.Device:  # [_make_device] 내부 전용 함수 정의
    """lightning.gpu → lightning.qubit → default.qubit 순 폴백."""
    if use_lightning:  # 조건이 참일 때만 실행한다  # Branch: executes only when condition is True
        ...
        try:  # 오류가 날 수 있는 코드 블록을 시도한다  # Try block: attempt code that might raise an exception
            ...
            pass  # 이것도 없으면 다음으로 넘어간다  # No-op placeholder
    return qml.device("default.qubit", wires=n_qubits)  # 기본 순수 파이썬 양자 시뮬레이터 사용  # Returns a value to the caller


def _build_qnode(  # [_build_qnode] 내부 전용 함수 정의
    ...
    def circuit(  # [circuit] 함수 정의 시작
        ...
        for k in range(n_qubits):  # 지정한 횟수만큼 반복한다  # Loop: iterate a fixed number of times
            qml.RY(theta[k], wires=k)   # k번째 큐비트를 theta[k] 각도만큼 Y축으로 회전시킨다
            qml.RZ(phi[k], wires=k)     # k번째 큐비트를 phi[k] 각도만큼 Z축으로 회전시킨다
        for idx, (i, j) in enumerate(qubit_pairs):  # 시퀀스에서 항목을 하나씩 꺼내어 반복한다  # Loop: iterate over each item in the sequence
            qml.IsingZZ(math.pi * J_upper[idx], wires=[i, j])  # i번째와 j번째 큐비트를 이징 상호작용으로 얽히게 만든다
        qml.StronglyEntanglingLayers(weights, wires=range(n_qubits))  # 학습 파라미터로 큐비트들을 강하게 얽히게 만든다  # Generates a sequence of integers
        return [qml.expval(qml.PauliZ(k)) for k in range(n_qubits)]  # 각 큐비트를 Z축 방향으로 측정한 기댓값 반환  # Returns a value to the caller

    return circuit  # 완성된 양자 회로 함수를 돌려준다  # Returns a value to the caller


def _build_batched_qnode(  # [_build_batched_qnode] 내부 전용 함수 정의
    dev: qml.Device,                         # 양자 디바이스
    n_qubits: int,                           # 큐비트 수
    n_vqc_layers: int,                       # VQC 레이어 수
    qubit_pairs: List[Tuple[int, int]],      # 큐비트 쌍 목록
    ...
    @qml.batch_params  # 여러 샘플을 한 번에 처리할 수 있게 한다  # Decorator: modifies the function / class below
    @qml.qnode(dev, interface="torch", diff_method="adjoint")  # PyTorch 연동 양자 회로
    def batched_circuit(  # [batched_circuit] 함수 정의 시작
        ...
        for idx, (i, j) in enumerate(qubit_pairs):  # 시퀀스에서 항목을 하나씩 꺼내어 반복한다  # Loop: iterate over each item in the sequence
            qml.IsingZZ(math.pi * J_upper[idx], wires=[i, j])  # 큐비트 쌍을 이징 상호작용으로 얽는다  # IsingZZ entangling gate between two qubits
        qml.StronglyEntanglingLayers(weights, wires=range(n_qubits))  # 강한 얽힘 레이어 적용  # Generates a sequence of integers
        return [qml.expval(qml.PauliZ(k)) for k in range(n_qubits)]  # Z축 기댓값 측정

    return batched_circuit  # 배치용 양자 회로 함수 반환  # Returns a value to the caller
...
class TorchVQC(nn.Module):  # ★ [TorchVQC] 클래스 정의 — 관련 데이터와 기능을 하나로 묶은 설계도
    """
    ...
    def __init__(self, n_vqc_layers: int = 2, n_qubits: int = N_QUBITS) -> None:
        ...
        self._qubit_pairs: List[Tuple[int, int]] = list(  # 리스트를 만들거나 다른 자료형을 리스트로 변환한다
            itertools.combinations(range(N), 2)  # 지정한 범위의 정수를 하나씩 만들어 낸다  # Generates a sequence of integers
            # N개의 큐비트 중 2개를 뽑는 모든 조합을 만든다 (N=3이면 (0,1),(0,2),(1,2) 총 3쌍)
        )
        n_pairs = len(self._qubit_pairs)  # 큐비트 쌍의 개수  # Returns the number of items

        # ── IsingZZ parity mask [n_pairs, D] ─────────────────────────────
        # 이징 상호작용에서 각 상태가 +1인지 -1인지를 미리 계산해둔다 (속도 향상)
        ising_masks = torch.zeros(n_pairs, D, dtype=torch.float32)  # 모든 값 0으로 초기화  # Creates a zero-filled tensor
        for p, (qi, qj) in enumerate(self._qubit_pairs):  # 시퀀스에서 항목을 하나씩 꺼내어 반복한다  # Loop: iterate over each item in the sequence
            # p번째 쌍 (qi, qj)에 대해 모든 상태 s를 검사한다
            for s in range(D):  # 지정한 횟수만큼 반복한다  # Loop: iterate a fixed number of times
        ...
        z_signs = torch.zeros(N, D, dtype=torch.float32)  # 모든 값 0으로 초기화  # Creates a zero-filled tensor
        for k in range(N):  # 지정한 횟수만큼 반복한다  # Loop: iterate a fixed number of times
        ...
        cnot_all = []
        for l in range(n_vqc_layers):  # 지정한 횟수만큼 반복한다  # Loop: iterate a fixed number of times
            rng = (l % (N - 1)) + 1 if N > 1 else 1  # 레이어마다 다른 간격으로 CNOT 연결 (링 토폴로지)
            layer = []
            for ctrl in range(N):  # 지정한 횟수만큼 반복한다  # Loop: iterate a fixed number of times
                tgt = (ctrl + rng) % N  # 제어 큐비트로부터 rng 간격 떨어진 큐비트가 타겟
                mat = torch.zeros(D, D, dtype=torch.complex64)  # D×D 복소수 행렬 초기화
                for s in range(D):  # 지정한 횟수만큼 반복한다  # Loop: iterate a fixed number of times
                    if (s >> (N - 1 - ctrl)) & 1:  # 조건이 참일 때만 실행한다  # Branch: executes only when condition is True
                        # 제어 큐비트가 1이면 타겟 큐비트를 뒤집는다 (XOR)
                        mat[s ^ (1 << (N - 1 - tgt)), s] = 1.0
                    else:  # 앞의 모든 조건이 거짓일 때 실행한다  # Branch: all previous conditions were False
                        mat[s, s] = 1.0  # 제어 큐비트가 0이면 타겟 큐비트는 그대로
                layer.append(mat)  # 리스트의 맨 뒤에 항목을 추가한다  # Appends an item to the end of the list
    ...
    def _ry(a: torch.Tensor) -> torch.Tensor:  # [_ry] 내부 전용 함수 정의
        """[B] → [B, 2, 2] complex64  RY(a) gate — Y축 회전 게이트 행렬을 만든다"""
        c, s = torch.cos(a * 0.5), torch.sin(a * 0.5)  # cos(a/2), sin(a/2) 계산
        return torch.stack([  # 함수의 계산 결과를 호출자에게 반환(돌려준다)  # Returns a value to the caller
            torch.stack([ c, -s], dim=-1),  # RY 행렬의 첫 번째 행: [cos, -sin]
            torch.stack([ s,  c], dim=-1),  # RY 행렬의 두 번째 행: [sin,  cos]
        ], dim=-2).to(torch.complex64)  # 복소수 타입으로 변환  # Moves tensor to specified device or dtype

    @staticmethod  # 객체(self) 없이 호출 가능한 정적 메서드로 만든다  # Decorator: static method — no self or cls
    def _rz(a: torch.Tensor) -> torch.Tensor:  # [_rz] 내부 전용 함수 정의
        """[B] → [B, 2, 2] complex64  RZ(a) gate — Z축 회전 게이트 행렬을 만든다"""
        h = a.to(torch.complex64) * 0.5  # a/2를 복소수로 변환
        z = torch.zeros_like(h)           # 0 텐서 (행렬의 0 원소)  # Creates a zero-filled tensor
        return torch.stack([  # 함수의 계산 결과를 호출자에게 반환(돌려준다)  # Returns a value to the caller
    ...
    @staticmethod  # 객체(self) 없이 호출 가능한 정적 메서드로 만든다  # Decorator: static method — no self or cls
    def _rot(w: torch.Tensor, B: int) -> torch.Tensor:  # [_rot] 내부 전용 함수 정의
        ...
        return torch.bmm(rz_omega, torch.bmm(ry_theta, rz_phi))  # 세 회전을 순서대로 합성한다  # Returns a value to the caller

    @staticmethod  # 객체(self) 없이 호출 가능한 정적 메서드로 만든다  # Decorator: static method — no self or cls
    def _apply_1q(  # [_apply_1q] 내부 전용 함수 정의
        ...
        B, D = state.shape   # 배치 크기와 상태 벡터 차원  # Shape (dimensions) of the tensor/array
        lo = 1 << (n - qubit - 1)  # qubit 아래쪽 비트들의 묶음 크기 (2^(n-qubit-1))
        hi = 1 << qubit            # qubit 위쪽 비트들의 묶음 크기 (2^qubit)
        s = state.reshape(B, hi, 2, lo)          # 상태를 [상위, 이 큐비트(2), 하위] 구조로 분리  # Reshapes tensor to a new shape
        s = s.permute(0, 2, 1, 3).reshape(B, 2, hi * lo)  # 이 큐비트 차원을 앞으로 이동  # Reshapes tensor to a new shape
        s = torch.bmm(gate, s)                   # 게이트 행렬을 상태에 곱한다 (양자 연산 적용)
        return s.reshape(B, 2, hi, lo).permute(0, 2, 1, 3).reshape(B, D)  # 원래 형태로 되돌린다  # Returns a value to the caller
    ...
    def forward(  # [forward] 신경망의 순방향 계산을 정의한다 (입력 → 출력)
        ...
        B  = theta.shape[0]   # 배치 크기  # Shape (dimensions) of the tensor/array
        N  = self.N           # 큐비트 수
        device = theta.device  # CPU인지 GPU인지

        # 초기 상태 |00…0⟩ — 모든 큐비트가 0인 상태에서 시작한다
        state = torch.zeros(B, self.D, dtype=torch.complex64, device=device)  # 전부 0으로 초기화  # Creates a zero-filled tensor
        ...
        for p in range(len(self._qubit_pairs)):  # 지정한 횟수만큼 반복한다  # Loop: iterate a fixed number of times
            phase = torch.exp(0.5j * phi_iz[:, p, None] * masks[p, None, :])  # 각 상태에 위상 곱하기  # Computes exponential e^x element-wise
            state = state * phase  # 이징 상호작용 적용 (상태에 위상을 곱한다)

        # 3. VQC + Fix B: Data Re-uploading — 학습 가능한 양자 회로 적용
        for l in range(self.n_vqc_layers):  # 지정한 횟수만큼 반복한다  # Loop: iterate a fixed number of times
            for k in range(N):  # 지정한 횟수만큼 반복한다  # Loop: iterate a fixed number of times
            ...
            if l < self.n_vqc_layers - 1:  # 조건이 참일 때만 실행한다  # Branch: executes only when condition is True
                for k in range(N):  # 지정한 횟수만큼 반복한다  # Loop: iterate a fixed number of times
        ...
        return expvals.to(dtype=theta.dtype)  # 입력과 같은 데이터 타입으로 변환해서 반환  # Returns a value to the caller
...
class QuantumHamiltonianLayer(nn.Module):  # ★ [QuantumHamiltonianLayer] 클래스 정의 — 관련 데이터와 기능을 하나로 묶은 설계도
    """
    ...
    def __init__(  # [__init__] 초기화 메서드 — 객체를 만들 때 가장 먼저 실행된다
        ...
        vqc_shape = qml.StronglyEntanglingLayers.shape(  # 텐서/배열의 형태(차원과 각 크기)를 확인한다  # Shape (dimensions) of the tensor/array
        ...
        self.vqc_weights = nn.Parameter(torch.randn(vqc_shape) * 0.01)  # 학습 중 자동 업데이트되는 파라미터로 등록한다  # Registers tensor as a learnable parameter (tracked by autograd)
        ...
        if use_spectral_norm:  # 조건이 참일 때만 실행한다  # Branch: executes only when condition is True
            from torch.nn.utils import spectral_norm as _sn  # 스펙트럴 정규화 도구 임포트  # Import spectral_norm as _sn from torch.nn.utils module
            self.logit_proj = nn.Sequential(  # 신경망 레이어를 만들어 이 객체에 저장한다  # Stores a neural network layer as an attribute of this module
                _sn(nn.Linear(n_qubits, 16)),  # 스펙트럴 정규화 적용 선형 레이어  # Fully-connected (affine) layer: y = xW^T + b
                nn.GELU(),                      # GELU 활성화 함수 (부드러운 ReLU)
                nn.Dropout(0.03),               # 3% 확률로 뉴런을 끄는 드롭아웃  # Dropout layer — randomly zeros activations to prevent overfitting
                _sn(nn.Linear(16, n_actions))   # 스펙트럴 정규화 적용 출력 레이어  # Fully-connected (affine) layer: y = xW^T + b
            )
            _classical = nn.Linear(n_qubits, n_actions, bias=True)  # 고전 경로 선형 레이어  # Fully-connected (affine) layer: y = xW^T + b
            ...
            self.logit_proj = nn.Sequential(  # 신경망 레이어를 만들어 이 객체에 저장한다  # Stores a neural network layer as an attribute of this module
                nn.Linear(n_qubits, 16),  # 큐비트 출력을 16차원으로 변환  # Fully-connected (affine) layer: y = xW^T + b
                nn.GELU(),                # GELU 활성화
                nn.Dropout(0.03),         # 드롭아웃  # Dropout layer — randomly zeros activations to prevent overfitting
                nn.Linear(16, n_actions)  # 16차원에서 행동 수(3)로 변환  # Fully-connected (affine) layer: y = xW^T + b
            )
            self.classical_head = nn.Linear(n_qubits, n_actions, bias=True)  # 고전 경로 선형 레이어  # Stores a neural network layer as an attribute of this module
        ...
        self.h_proj = nn.Linear(n_qubits, n_qubits, bias=False)  # 신경망 레이어를 만들어 이 객체에 저장한다  # Stores a neural network layer as an attribute of this module
        ...
        self.logit_scale = nn.Parameter(torch.ones(1) * 3.0)  # 로짓 스케일 파라미터 (초기값 3.0)  # Registers tensor as a learnable parameter (tracked by autograd)

    def _run_circuit_batch(  # [_run_circuit_batch] 내부 전용 함수 정의
        self,
        ...
        j_upper = torch.stack(  # 여러 텐서를 새 차원으로 쌓는다  # Stacks tensors along a new dimension
            [J[:, i, j] for (i, j) in self._qubit_pairs], dim=1
            # 전체 K×K 행렬에서 큐비트 쌍에 해당하는 값들만 꺼내서 쌓는다
        )  # [B, n_pairs]

        n_cache = min(8, theta.shape[0])  # 최대 8개 샘플만 QFI 캐시에 저장한다  # Shape (dimensions) of the tensor/array
        self._qfi_cache = (
            theta[:n_cache].detach(),    # QFI 계산용 각도 캐시 (기울기 그래프에서 분리)
            phi[:n_cache].detach(),  # 기울기 계산 그래프에서 분리한다 (값만 복사)  # Detaches tensor from the computation graph
            j_upper[:n_cache].detach(),  # 기울기 계산 그래프에서 분리한다 (값만 복사)  # Detaches tensor from the computation graph
        )

        return self._torch_vqc(theta, phi, j_upper, self.vqc_weights)  # 양자 회로를 실행하고 측정값 반환  # Returns a value to the caller

    @torch.no_grad()  # 기울기 계산을 끄고 실행 (빠른 연산)  # Decorator: modifies the function / class below
    def compute_qfi_diagonal(self) -> Optional[torch.Tensor]:  # [compute_qfi_diagonal] 함수 정의 시작
        """Diagonal QFI via parameter-shift rule — 파라미터 이동 규칙으로 대각 양자 피셔 정보를 계산한다."""
        if not self._qfi_cache:  # 조건이 거짓일 때만 실행한다  # Branch: executes only when condition is False
            return None  # 캐시가 없으면 계산 불가  # Returns a value to the caller
        ...
        w_flat   = self.vqc_weights.detach().clone().reshape(-1)  # 1차원으로 펼친 파라미터  # Reshapes tensor to a new shape
        qfi_diag = torch.zeros(n_params, device=self.vqc_weights.device)  # QFI 대각 원소들
        half_pi  = math.pi / 2.0  # π/2 상수

        for i in range(n_params):  # 지정한 횟수만큼 반복한다  # Loop: iterate a fixed number of times
            w_plus      = w_flat.clone(); w_plus[i]  += half_pi   # i번째 파라미터를 +π/2 이동
            w_minus     = w_flat.clone(); w_minus[i] -= half_pi   # i번째 파라미터를 -π/2 이동
            out_plus    = self._torch_vqc(theta, phi, j_upper, w_plus.reshape(w_shape))   # +이동 후 측정  # Reshapes tensor to a new shape
            out_minus   = self._torch_vqc(theta, phi, j_upper, w_minus.reshape(w_shape))  # -이동 후 측정  # Reshapes tensor to a new shape
    ...
    def forward(  # [forward] 신경망의 순방향 계산을 정의한다 (입력 → 출력)
        ...
        Returns:
            ...
            J       : [B, K, K]
        """
        B, T, K = c_kt.shape  # 배치 크기, 시간 길이, 큐비트 수  # Shape (dimensions) of the tensor/array
        ...
        if last_step_only:  # 조건이 참일 때만 실행한다  # Branch: executes only when condition is True
        ...
        expvals_flat = expvals_flat.to(dtype=theta_flat.dtype)  # 입력과 같은 dtype으로 맞춘다  # Moves tensor to specified device or dtype
        expvals      = expvals_flat.reshape(B, T_run, K)  # [B, T_run, K] 다시 배치와 시간으로 분리

        h_broadcast = h_field.unsqueeze(1).expand(-1, T_run, -1)  # 외부 자기장을 시간 축으로 복사  # Inserts a new dimension of size 1
        ...
        return logits, expvals, J  # 로짓, 측정값, 상호작용 행렬을 돌려준다  # Returns a value to the caller
...
class QuantumMarketEncoder(nn.Module):  # ★ [QuantumMarketEncoder] 클래스 정의 — 관련 데이터와 기능을 하나로 묶은 설계도
    ...
    def __init__(  # [__init__] 초기화 메서드 — 객체를 만들 때 가장 먼저 실행된다
        ...
        from src.data.spectral_decomposer import SpectralDecomposer  # 스펙트럴 분해기 임포트  # Import SpectralDecomposer from src.data.spectral_decomposer module
        ...
        if use_transformer:  # 조건이 참일 때만 실행한다  # Branch: executes only when condition is True
        ...
        if n_eigenvectors != N_QUBITS:  # 조건이 참일 때만 실행한다  # Branch: executes only when condition is True
            self.proj_to_qubits: Optional[nn.Linear] = nn.Linear(  # 선형 변환(행렬 곱 + 편향) 레이어를 만든다  # Fully-connected (affine) layer: y = xW^T + b
    ...
    def forward(  # [forward] 신경망의 순방향 계산을 정의한다 (입력 → 출력)
        ...
        if self.temporal_encoder is not None:  # 조건이 참일 때만 실행한다  # Branch: executes only when condition is True
            c_kt = self.temporal_encoder(c_kt)  # Transformer로 시계열 컨텍스트 강화
        # Project to qubit dim if n_eigenvectors != n_qubits
        if self.proj_to_qubits is not None:  # 조건이 참일 때만 실행한다  # Branch: executes only when condition is True
        ...
        return logits, expvals, J, c_kt  # 로짓, 측정값, 상호작용, 고유 성분 반환  # Returns a value to the caller
...
def build_quantum_layer(  # [build_quantum_layer] 함수 정의 시작
    ...
    return QuantumHamiltonianLayer(  # 함수의 계산 결과를 호출자에게 반환(돌려준다)  # Returns a value to the caller
        n_qubits=n_qubits, n_vqc_layers=n_vqc_layers,
        n_actions=n_actions, use_lightning=use_lightning,
    ).to(device)  # 생성한 레이어를 지정한 디바이스로 이동한다  # Moves tensor to specified device or dtype


def build_quantum_market_encoder(  # [build_quantum_market_encoder] 함수 정의 시작
    feature_dim:    int  = 27,
    ...
    if device is None:  # 조건이 참일 때만 실행한다  # Branch: executes only when condition is True
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # 디바이스 자동 선택  # Target compute device: CUDA GPU or CPU
    return QuantumMarketEncoder(  # 함수의 계산 결과를 호출자에게 반환(돌려준다)  # Returns a value to the caller
    ...
    ).to(device)  # 생성한 인코더를 지정 디바이스로 이동  # Moves tensor to specified device or dtype
...
if __name__ == "__main__":  # 조건이 참일 때만 실행한다  # Branch: executes only when condition is True
    print("=" * 65)  # 구분선 출력  # Prints output to stdout
    print("  QuantumHamiltonianLayer — Dynamic N-Qubit Self-Test")  # 테스트 제목  # Prints output to stdout
    print("=" * 65)  # 결과를 화면에 출력한다  # Prints output to stdout
    torch.manual_seed(42)  # 난수 시드 고정 (재현 가능성 확보)

    for n_q in [2, 4]:  # 큐비트 2개와 4개 두 가지 경우를 테스트  # Loop: iterate over each item in the sequence
        print(f"\n[N_QUBITS={n_q}]")  # 현재 테스트하는 큐비트 수 출력  # Prints output to stdout
        ql = QuantumHamiltonianLayer(n_qubits=n_q, n_vqc_layers=2, n_actions=3)  # 레이어 생성
        pairs = ql._qubit_pairs  # 큐비트 쌍 목록
        print(f"  qubit_pairs = {pairs}  ({len(pairs)} pairs)")   # 큐비트 쌍 출력  # Prints output to stdout
        print(f"  vqc_weights = {tuple(ql.vqc_weights.shape)}")   # VQC 파라미터 형태 출력
        print(f"  logit_proj  = Linear({n_q}, 3)")                 # 로짓 레이어 형태 출력  # Prints output to stdout

        B, T = 2, 20  # 배치 크기 2, 시간 길이 20
        c  = torch.randn(B, T, n_q)   # 임의 고유 성분 생성  # Creates a tensor with standard normal random values
        dc = torch.randn(B, T, n_q)   # 임의 변화율 생성  # Creates a tensor with standard normal random values
        logits, expvals, J = ql(c, dc, last_step_only=True)  # 마지막 타임스텝만 처리

        assert logits.shape  == (B, 1, 3),    f"logits shape wrong: {logits.shape}"    # 로짓 형태 검증  # Assertion: raises AssertionError if condition is False
        assert expvals.shape == (B, 1, n_q),  f"expvals shape wrong: {expvals.shape}"  # 측정값 형태 검증  # Assertion: raises AssertionError if condition is False
        assert J.shape       == (B, n_q, n_q),f"J shape wrong: {J.shape}"              # 상호작용 행렬 형태 검증  # Assertion: raises AssertionError if condition is False

        logits.sum().backward()  # 역전파 실행 (기울기 계산)  # Computes gradients via backpropagation
        grad = ql.vqc_weights.grad.norm().item()  # VQC 파라미터의 기울기 크기
        print(f"  logits={tuple(logits.shape)} expvals={tuple(expvals.shape)} J={tuple(J.shape)}")  # 형태 출력  # Shape (dimensions) of the tensor/array
        print(f"  VQC grad norm = {grad:.4f}  (0이면 BARREN)")  # 기울기가 0이면 배런 플래토 문제  # Prints output to stdout
        ...
        print(f"  T=1 J.abs().mean = {J_t1.abs().mean().item():.4f}  (캐시 재사용)")  # 캐시가 잘 작동하는지 확인  # Extracts a Python scalar from a 1-element tensor

    print("\n  ALL CHECKS PASSED")  # 모든 검사 통과  # Prints output to stdout
    print("=" * 65)  # 결과를 화면에 출력한다  # Prints output to stdout
