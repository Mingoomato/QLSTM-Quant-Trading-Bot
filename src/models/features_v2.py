...
def compute_true_atr(  # [compute_true_atr] Function definition
    ...
    """
    n = len(closes)  # 종가 데이터의 개수를 n에 저장한다  # Returns the number of items
    if n < 2:  # 데이터가 2개보다 적으면  # Branch: executes only when condition is True
        return np.zeros(n)  # 0으로 채워진 배열을 반환한다 (계산 불가)  # Returns a value to the caller
    ...
    tr[1:] = np.maximum(h - l, np.maximum(np.abs(h - pc), np.abs(l - pc)))  # Finds the maximum value
    ...
    return atr  # 계산된 ATR 배열을 반환한다  # Returns a value to the caller


# ── V2 per-bar feature builder ────────────────────────────────────

def build_features_v2(df: pd.DataFrame) -> np.ndarray:  # [build_features_v2] Function definition
    ...
    volumes = df["volume"].values.astype(float) if "volume" in df.columns \
              else np.ones(len(df))  # Creates an array filled with ones
    n = len(closes)  # 종가 데이터 개수를 n에 저장한다  # Returns the number of items
    eps = 1e-8  # 0으로 나누는 오류를 막기 위한 아주 작은 숫자를 eps에 저장한다

    # ── Log returns ──────────────────────────────────────────────
    if n >= 2:  # 데이터가 2개 이상이면  # Branch: executes only when condition is True
        # 오늘 종가를 어제 종가로 나눈 뒤 로그를 취해 수익률을 계산한다
        log_rets = np.diff(np.log(np.where(closes > 0, closes, eps)))  # Computes natural logarithm element-wise
    else:  # 데이터가 1개뿐이면  # Branch: all previous conditions were False
        log_rets = np.array([0.0])  # 수익률을 0으로 설정한다  # Converts Python sequence to NumPy array

    def lr(k: int) -> float:  # [lr] Function definition
        # k번째 이전 봉의 로그 수익률을 반환한다. 데이터가 없으면 0을 반환한다
        return float(log_rets[-(k + 1)]) if len(log_rets) >= k + 1 else 0.0  # Returns a value to the caller
    ...
    v_window  = volumes[-20:] if len(volumes) >= 20 else volumes  # 최근 20개 거래량을 가져온다  # Returns the number of items
    v_mean    = float(np.mean(v_window)) if len(v_window) > 0 else 1.0  # 평균 거래량을 계산한다  # Computes the mean value
    # 현재 거래량을 평균 거래량으로 나눈 뒤 로그를 취해 거래량 비율을 계산한다
    vol_ratio = float(np.log(max(volumes[-1], eps) / max(v_mean, eps)))  # Computes natural logarithm element-wise

    # ── EMA deviations (log-scale) ────────────────────────────────
    def ema_val(arr: np.ndarray, span: int) -> float:  # [ema_val] Function definition
        # 주어진 배열에 지수 이동 평균(EMA)을 계산하는 함수다
        if len(arr) == 0:  # 배열이 비어있으면  # Branch: executes only when condition is True
        ...
        for x in arr[1:]:  # 두 번째 값부터 마지막 값까지 반복한다  # Loop: iterate over each item in the sequence
            e = alpha * float(x) + (1.0 - alpha) * e  # 새 값과 이전 EMA를 가중 평균한다
        return e  # 최종 EMA 값을 반환한다  # Returns a value to the caller
    ...
    ema12_dev = float(np.log(max(closes[-1], eps) / max(ema12, eps)))  # Computes natural logarithm element-wise
    # 현재 종가가 EMA26보다 얼마나 위/아래에 있는지 로그로 계산한다
    ema26_dev = float(np.log(max(closes[-1], eps) / max(ema26, eps)))  # Computes natural logarithm element-wise
    macd_val  = ema12_dev - ema26_dev  # MACD = EMA12 편차 - EMA26 편차로 계산한다

    # ── RSI (14-bar on log returns, normalized 0-1) ───────────────
    if len(log_rets) >= 14:  # 로그 수익률이 14개 이상 있을 때  # Branch: executes only when condition is True
        lr14   = log_rets[-14:]  # 최근 14개의 로그 수익률을 가져온다
        gains  = np.where(lr14 > 0, lr14, 0.0)   # 양수 수익률만 남긴다 (오른 날)  # Returns elements chosen from two arrays by condition
        losses = np.where(lr14 < 0, -lr14, 0.0)  # 음수 수익률의 절댓값만 남긴다 (내린 날)  # Returns elements chosen from two arrays by condition
        ag  = float(np.mean(gains))   # 평균 상승폭을 계산한다  # Computes the mean value
        al  = float(np.mean(losses))  # 평균 하락폭을 계산한다  # Computes the mean value
        rs  = ag / max(al, eps)  # RS = 평균 상승폭 / 평균 하락폭으로 계산한다
        rsi = 1.0 - 1.0 / (1.0 + rs)  # RSI = 1 - 1/(1+RS) 공식으로 0~1 범위로 계산한다
    else:  # 데이터가 14개 미만이면  # Branch: all previous conditions were False
    ...
    def mom(k: int) -> float:  # [mom] Function definition
        # k봉 전 대비 현재 종가의 로그 수익률(모멘텀)을 계산하는 함수다
        # k봉 모멘텀: 현재 종가 / k봉 전 종가의 로그 수익률. 데이터 부족 시 0 반환
    ...
    # ── Stochastic %K (14-bar) ────────────────────────────────────
    if n >= 14:  # 데이터가 14개 이상이면  # Branch: executes only when condition is True
        lo14    = float(np.min(lows[-14:]))    # 최근 14봉 중 가장 낮은 저가를 구한다  # Finds the minimum value
        hi14    = float(np.max(highs[-14:]))   # 최근 14봉 중 가장 높은 고가를 구한다  # Finds the maximum value
    ...
    if len(log_rets) >= 10:  # 로그 수익률이 10개 이상 있을 때  # Branch: executes only when condition is True
        signs = np.sign(log_rets[-10:])  # 최근 10개 수익률의 부호(+1, 0, -1)를 구한다
        # 부호가 바뀌는 횟수를 세어 9로 나눠 0~1 범위의 방향 전환 빈도를 계산한다
        sc    = float(np.sum(np.abs(np.diff(signs)) > 0)) / 9.0  # Sums array elements
    else:  # 데이터가 10개 미만이면  # Branch: all previous conditions were False
        sc = 0.0  # 방향 전환 빈도를 0으로 설정한다

    # ── Trend strength: local Sharpe of log returns (10-bar) ──────
    if len(log_rets) >= 10:  # 로그 수익률이 10개 이상 있을 때  # Branch: executes only when condition is True
        mu10    = float(np.mean(log_rets[-10:]))  # 최근 10봉 수익률의 평균을 구한다  # Computes the mean value
        sig10   = float(np.std(log_rets[-10:]))   # 최근 10봉 수익률의 표준편차를 구한다  # Computes the standard deviation
    ...
    if hasattr(df.index[-1], "hour"):  # 인덱스에 hour 속성이 있으면 (datetime 형식이면)  # Branch: executes only when condition is True
        hour = int(df.index[-1].hour)  # 마지막 봉의 시(hour)를 꺼낸다
    elif "ts" in df.columns:  # 'ts' 열이 있으면 (타임스탬프 열)  # Branch: previous condition was False, try this one
        ts_val = df["ts"].iloc[-1]  # 마지막 행의 타임스탬프 값을 가져온다
        if isinstance(ts_val, (int, float, np.integer, np.floating)):  # 숫자형이면  # Branch: executes only when condition is True
    ...
    feat = np.array([  # Converts Python sequence to NumPy array
        lr0,       # 0  현재 봉의 로그 수익률
        ...
        rsi,       # 6  RSI 14봉 (0~1)
        atr,       # 7  True ATR / 종가  # ATR: Average True Range — average price volatility
    ...
    return np.clip(feat, -np.pi, np.pi)  # 모든 값을 -π ~ +π 범위로 잘라서 반환한다  # Returns a value to the caller

# ── Disk-cached feature matrix builder ───────────────────────────

def generate_and_cache_features_v2(  # [generate_and_cache_features_v2] Function definition
    ...
    if os.path.exists(cache_path):  # 캐시 파일이 이미 존재하면  # Branch: executes only when condition is True
        ...
        if len(cached) == len(df_clean):  # 캐시 길이가 현재 데이터 길이와 같으면  # Branch: executes only when condition is True
            if verbose:  # 출력 옵션이 켜져 있으면  # Branch: executes only when condition is True
                print(f"[features_v2] Loaded cache: {cache_path}")  # 캐시 불러왔다고 알린다  # Prints output to stdout
            return cached  # 캐시 데이터를 바로 반환한다 (새로 계산하지 않는다)  # Returns a value to the caller
        if verbose:  # 크기가 달라서 캐시를 다시 만들어야 할 때  # Branch: executes only when condition is True
            # Prints output to stdout
            print(f"[features_v2] Cache size mismatch ({len(cached)} vs {len(df_clean)}), rebuilding...")

    if verbose:  # 출력 옵션이 켜져 있으면  # Branch: executes only when condition is True
        print(f"[features_v2] Building {len(df_clean)} V2 feature vectors...")  # 피처 생성 시작을 알린다  # Prints output to stdout
    ...
    for i in iterator:  # 0번 봉부터 마지막 봉까지 하나씩 반복한다  # Loop: iterate over each item in the sequence
        if i < warmup:  # 워밍업 구간이면 (아직 충분한 데이터가 없을 때)  # Branch: executes only when condition is True
            feature_list.append(np.zeros(16, dtype=np.float32))  # 16개의 0으로 채워진 피처를 추가한다  # Creates a zero-filled array
        ...
        feature_list.append(build_features_v2(window))  # 그 창에서 V2 피처를 계산해 리스트에 추가한다  # Appends an item to the end of the list
    ...
    if verbose:  # 출력 옵션이 켜져 있으면  # Branch: executes only when condition is True
        print(f"[features_v2] Saved cache: {cache_path}")  # 저장 완료를 알린다  # Prints output to stdout
