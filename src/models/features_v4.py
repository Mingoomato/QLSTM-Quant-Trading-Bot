...
V4 Feature Index:
  0-15 : V2 base (16-dim)
    16  : funding_rate_zscore
    17  : candle_body_ratio
    18  : volume_zscore
    19  : oi_change_pct
    20  : funding_velocity
    21  : cvd_delta_zscore       ← CVD Tier-1
    22  : cvd_trend_zscore       ← CVD Tier-1
    23  : cvd_price_divergence   ← CVD Tier-1
    24  : liq_long_zscore        ← Liquidation proxy
    25  : liq_short_zscore       ← Liquidation proxy

Total: 26-dim
NOTE: Hurst exponent and purity_proxy (v3 statistical features) removed 2026-03-23.
...
from src.models.features_v2 import build_features_v2  # V2 base features (16-dim)


# ── V4 extra feature helpers ──────────────────────────────────────────────────

def _funding_zscore(funding_series: np.ndarray, window: int = 30) -> float:  # [_funding_zscore] Private helper function
    """Z-score of the most recent funding rate vs rolling window."""
    if len(funding_series) < 2:  # 데이터가 2개 미만이면 계산 불가  # Branch: executes only when condition is True
        return 0.0  # 0을 반환한다  # Returns a value to the caller
    w = min(len(funding_series), window)  # 실제 사용할 창 크기를 결정한다 (데이터 길이와 window 중 작은 쪽)  # Returns the number of items
...
def _candle_body_ratio(opens: np.ndarray, highs: np.ndarray,  # [_candle_body_ratio] Private helper function
                       lows: np.ndarray, closes: np.ndarray,
                       smooth: int = 3) -> float:
    """EMA-smoothed candle body ratio over last `smooth` bars."""
    n = len(closes)  # 데이터 개수를 n에 저장한다  # Returns the number of items
    if n < 1:  # 데이터가 없으면  # Branch: executes only when condition is True
        return 0.0  # 0을 반환한다  # Returns a value to the caller
    k = min(n, smooth)  # 실제 사용할 봉 수를 결정한다 (데이터 길이와 smooth 중 작은 쪽)
    ratios = []  # 각 봉의 몸통 비율을 저장할 빈 리스트를 만든다
    for i in range(-k, 0):  # 최근 k개 봉에 대해 반복한다  # Loop: iterate a fixed number of times
        hl = highs[i] - lows[i] + 1e-8  # 봉의 고가-저가 범위를 계산한다 (0 방지용 작은 수 추가)
        ratio = (closes[i] - opens[i]) / hl  # 몸통 비율 = (종가-시가) / (고가-저가)
        ratios.append(float(np.clip(ratio, -1.0, 1.0)))  # -1~1로 잘라서 리스트에 추가한다  # Clips values to [min, max] range
    # 3개 이하에서는 단순 평균이 EMA에 근사한다
    return float(np.mean(ratios))  # 평균 몸통 비율을 반환한다  # Returns a value to the caller


def _volume_zscore(volumes: np.ndarray, window: int = 20) -> float:  # [_volume_zscore] Private helper function
    """Volume z-score over rolling window, clipped to [-3, 3]."""
    if len(volumes) < 2:  # 데이터가 2개 미만이면  # Branch: executes only when condition is True
        return 0.0  # 0을 반환한다  # Returns a value to the caller
    w = min(len(volumes), window)  # 실제 사용할 창 크기를 결정한다  # Returns the number of items
...
def _oi_change_pct(oi_series: np.ndarray, window: int = 5) -> float:  # [_oi_change_pct] Private helper function
    ...
    """
    if len(oi_series) < window + 1:  # 데이터가 window+1개 미만이면 계산 불가  # Branch: executes only when condition is True
        return 0.0  # 0을 반환한다  # Returns a value to the caller
    oi_prev = oi_series[-(window + 1)]  # window봉 전의 미결제약정(OI) 값을 가져온다
    oi_curr = oi_series[-1]  # 현재 미결제약정(OI) 값을 가져온다
    if oi_prev <= 0:  # 이전 OI가 0 이하이면 계산 불가  # Branch: executes only when condition is True
        return 0.0  # 0을 반환한다  # Returns a value to the caller
    change = (oi_curr - oi_prev) / (oi_prev + 1e-10)  # OI 변화율(%) = (현재-이전) / 이전
    # 최근 30봉의 OI 변화를 기준으로 정상화(z-score)한다
    hist_len = min(len(oi_series) - 1, 30)  # 사용할 과거 데이터 길이를 결정한다  # Returns the number of items
    if hist_len < 2:  # 과거 데이터가 2개 미만이면  # Branch: executes only when condition is True
        return float(np.clip(change * 10.0, -np.pi, np.pi))  # 단순 스케일링해서 반환한다  # Returns a value to the caller
    hist = oi_series[-(hist_len + 1):]  # 과거 hist_len+1개의 OI 데이터를 가져온다
    changes = np.diff(hist) / (hist[:-1] + 1e-10)  # 연속 봉 간 OI 변화율을 계산한다  # Computes differences between consecutive elements
...
# [_funding_velocity] Private helper function
def _funding_velocity(funding_series: np.ndarray, window: int = 8) -> float:
    ...
    """
    if len(funding_series) < 2:  # 데이터가 2개 미만이면  # Branch: executes only when condition is True
        return 0.0  # 0을 반환한다  # Returns a value to the caller
    w = min(len(funding_series), window)  # 실제 사용할 창 크기를 결정한다  # Returns the number of items
    delta = float(funding_series[-1]) - float(funding_series[-w])  # 현재 펀딩 비율 - window봉 전 펀딩 비율
    std = np.std(funding_series[-min(len(funding_series), 30):]) + 1e-10  # 최근 30봉 펀딩 비율의 표준편차  # Computes the standard deviation
    return float(np.clip(delta / std, -np.pi, np.pi))  # 변화속도를 표준편차로 나눈 뒤 -π~π로 잘라서 반환한다  # Returns a value to the caller


# ── CVD (Cumulative Volume Delta) helpers ─────────────────────────────────────

def _compute_bar_deltas(highs: np.ndarray, lows: np.ndarray,  # [_compute_bar_deltas] Private helper function
    ...
    return volumes * (2.0 * closes - highs - lows) / hl  # Returns a value to the caller


def _cvd_delta_zscore(deltas: np.ndarray, window: int = 20) -> float:  # [_cvd_delta_zscore] Private helper function
    """Z-score of the most recent bar's delta vs rolling window.

    Instantaneous buy/sell pressure relative to recent baseline.
    """
    if len(deltas) < 2:  # 데이터가 2개 미만이면  # Branch: executes only when condition is True
        return 0.0  # 0을 반환한다  # Returns a value to the caller
    w   = min(len(deltas), window)  # 실제 사용할 창 크기를 결정한다  # Returns the number of items
...
def _cvd_trend_zscore(deltas: np.ndarray, trend_window: int = 20,  # [_cvd_trend_zscore] Private helper function
    ...
    """
    if len(deltas) < trend_window:  # 데이터가 trend_window개 미만이면  # Branch: executes only when condition is True
        return 0.0  # 0을 반환한다  # Returns a value to the caller
    # norm_window개의 롤링 trend_window봉 누적합을 계산한다
    n_sums = min(len(deltas) - trend_window + 1, norm_window)  # 사용할 누적합 개수를 결정한다  # Returns the number of items
    if n_sums < 2:  # 누적합이 2개 미만이면  # Branch: executes only when condition is True
    ...
    # 여러 시점의 누적합을 배열로 만든다
    sums = np.array([  # Converts Python sequence to NumPy array
...
def _cvd_price_divergence(deltas: np.ndarray, volumes: np.ndarray,  # [_cvd_price_divergence] Private helper function
    ...
    """
    if len(closes) < window + 1 or len(deltas) < window:  # 데이터가 충분하지 않으면  # Branch: executes only when condition is True
        return 0.0  # 0을 반환한다  # Returns a value to the caller
    ...
    ret_std    = float(np.std(np.diff(np.log(closes[-window:] + 1e-10))) + 1e-10)  # 수익률 표준편차  # Computes natural logarithm element-wise
    price_dir  = log_ret / (ret_std * np.sqrt(window) + 1e-10)  # 가격 방향 = 수익률 / 표준화 스케일  # Computes square root element-wise

    divergence = float(np.tanh((cvd_dir - price_dir) * 2.0))  # CVD방향과 가격방향의 차이를 tanh로 압축
    return float(np.clip(divergence, -np.pi, np.pi))  # 결과를 -π~π로 잘라서 반환한다  # Returns a value to the caller


def _liq_long_zscore(highs: np.ndarray, lows: np.ndarray,  # [_liq_long_zscore] Private helper function
    ...
    lower_wicks = np.maximum(0.0, np.minimum(opens, closes) - lows) * volumes  # Finds the maximum value
    if len(lower_wicks) < 2:  # 데이터가 2개 미만이면  # Branch: executes only when condition is True
        return 0.0  # 0을 반환한다  # Returns a value to the caller
    w   = min(len(lower_wicks), window)  # 실제 사용할 창 크기를 결정한다  # Returns the number of items
...
def _liq_short_zscore(highs: np.ndarray, lows: np.ndarray,  # [_liq_short_zscore] Private helper function
                      opens: np.ndarray, closes: np.ndarray,
                      volumes: np.ndarray, window: int = 20) -> float:
    ...
    upper_wicks = np.maximum(0.0, highs - np.maximum(opens, closes)) * volumes  # Finds the maximum value
    if len(upper_wicks) < 2:  # 데이터가 2개 미만이면  # Branch: executes only when condition is True
        return 0.0  # 0을 반환한다  # Returns a value to the caller
    w   = min(len(upper_wicks), window)  # 실제 사용할 창 크기를 결정한다  # Returns the number of items
...
def build_features_v4(df: pd.DataFrame) -> np.ndarray:  # [build_features_v4] Function definition
    """
    ...
    Returns float32 [28].
    """
    # ── Base features: V2 (16-dim) ──────────────────────────────────────────────
    base = build_features_v2(df)  # 16-dim V2 features
    ...
    if "funding_rate" in df.columns:  # 펀딩 비율 열이 있으면  # Branch: executes only when condition is True
        funding = df["funding_rate"].values.astype(float)  # 펀딩 비율 데이터를 꺼낸다  # Funding rate: periodic payment between longs and shorts
        f20 = _funding_zscore(funding, window=30)  # 펀딩 비율 z-점수를 계산한다
    else:  # 펀딩 비율 열이 없으면  # Branch: all previous conditions were False
    ...
    if "open_interest" in df.columns:  # 미결제약정(OI) 열이 있으면  # Branch: executes only when condition is True
    ...
    if funding is not None:  # 펀딩 비율 데이터가 있으면  # Branch: executes only when condition is True
        f24 = _funding_velocity(funding, window=8)  # 8봉 펀딩 비율 변화 속도를 계산한다
    else:  # 펀딩 비율 데이터가 없으면  # Branch: all previous conditions were False
        f24 = 0.0  # 피처 값을 0으로 설정한다

    # ── CVD features (feat 25–27) ─────────────────────────────────────────
    # 우선순위: Binance의 실제 taker_buy_volume이 있으면 진짜 CVD를 사용한다
    # 없으면: OHLCV만으로 근사 계산한다
    if "taker_buy_volume" in df.columns:  # 실제 매수 테이커 거래량 열이 있으면  # Branch: executes only when condition is True
    ...
    else:  # 실제 taker 데이터가 없으면  # Branch: all previous conditions were False
    ...
    extra = np.array([  # Converts Python sequence to NumPy array
        float(np.clip(f20,        -np.pi, np.pi)),  # 펀딩 비율 z-점수를 -π~π로 잘라 저장한다  # Clips values to [min, max] range
        float(np.clip(f21,        -np.pi, np.pi)),  # 봉 몸통 비율을 -π~π로 잘라 저장한다  # Clips values to [min, max] range
        float(np.clip(f22,        -np.pi, np.pi)),  # 거래량 z-점수를 -π~π로 잘라 저장한다  # Clips values to [min, max] range
        float(np.clip(f23,        -np.pi, np.pi)),  # OI 변화율을 -π~π로 잘라 저장한다
        float(np.clip(f24,        -np.pi, np.pi)),  # 펀딩 변화 속도를 -π~π로 잘라 저장한다  # Clips values to [min, max] range
        ...
        float(np.clip(f_liq_long, -np.pi, np.pi)),   # 롱 청산 z-점수를 -π~π로 잘라 저장한다 (28-dim 배열의 26번째 인덱스)  # Clips values to [min, max] range
        float(np.clip(f_liq_short,-np.pi, np.pi)),   # 숏 청산 z-점수를 -π~π로 잘라 저장한다 (28-dim 배열의 27번째 인덱스)  # Clips values to [min, max] range
...
def generate_and_cache_features_v4(  # [generate_and_cache_features_v4] Function definition
    ...
    """
    FEAT_DIM = 26  # V4 피처 차원 수: V2(16) + V4 extras(10)
    if os.path.exists(cache_path):  # 캐시 파일이 이미 존재하면  # Branch: executes only when condition is True
        ...
        if cached.shape == (len(df_clean), FEAT_DIM):  # 캐시의 형태가 현재 데이터와 맞으면  # Branch: executes only when condition is True
            if verbose:  # 출력 옵션이 켜져 있으면  # Branch: executes only when condition is True
                print(f"[features_v4] Loaded cache: {cache_path}  shape={cached.shape}")  # 캐시 불러왔다고 알린다  # Shape (dimensions) of the tensor/array
    ...
    if verbose:  # 출력 옵션이 켜져 있으면  # Branch: executes only when condition is True
        print(  # Prints output to stdout
            ...
            f"| funding_rate={'YES' if has_funding else 'NO'} "  # 펀딩 비율 데이터 존재 여부 출력  # Funding rate: periodic payment between longs and shorts
    ...
    iterator = (
        tqdm(range(n), desc="Building V4 Features", ascii=True)  # Generates a sequence of integers
        if verbose else range(n)  # Branch: executes only when condition is True
    )
    for i in iterator:  # 0번 봉부터 마지막 봉까지 하나씩 반복한다  # Loop: iterate over each item in the sequence
        if i < warmup:  # 워밍업 구간이면 (아직 충분한 데이터가 없을 때)  # Branch: executes only when condition is True
            feature_list.append(np.zeros(FEAT_DIM, dtype=np.float32))  # 28개의 0으로 채워진 피처를 추가한다  # Creates a zero-filled array
        ...
        feature_list.append(build_features_v4(window))  # 그 창에서 V4 피처를 계산해 리스트에 추가한다  # Appends an item to the end of the list
    ...
    if verbose:  # 출력 옵션이 켜져 있으면  # Branch: executes only when condition is True
        print(f"[features_v4] Saved: {cache_path}  shape={all_features.shape}")  # 저장 완료를 알린다  # Shape (dimensions) of the tensor/array
