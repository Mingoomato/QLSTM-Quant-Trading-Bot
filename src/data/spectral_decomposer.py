...
class SpectralDecomposer(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("_lda_W", torch.zeros(feature_dim, 3))
    ...
    @staticmethod
    def _covariance_matrix(x_norm: torch.Tensor) -> torch.Tensor:
        """
        공분산 행렬 Σ 계산.
    ...
    def _marchenko_pastur_denoise(
        ...
        self._last_rmt_threshold = lambda_plus.detach().mean().unsqueeze(0)
        ...
        lambda_denoised = (
            ...
            + lambda_noise.unsqueeze(-1) * noise_mask  # 노이즈 고유값은 평탄한 바닥값으로
        )                                                         # [B, F]
        ...
        cov_denoised = torch.bmm(
            ...
            eigenvectors_asc.transpose(-1, -2),
        )                                                         # [B, F, F]

        # 수치 오차로 대칭성이 깨지는 것을 방지: (A + A^T)/2로 완전히 대칭으로 만든다
        cov_denoised = (cov_denoised + cov_denoised.transpose(-1, -2)) / 2.0
    ...
    def _eigen_decompose(
        ...
        self._last_eigenvalues  = top_eigenvalues.detach()
        # 나중에 설명이나 디버깅에 쓸 수 있도록 고유벡터를 캐시에 저장한다
        self._last_eigenvectors = top_eigenvectors.detach()
    ...
    def fit_lda(self, X: "np.ndarray", y: "np.ndarray") -> None:  # noqa: F821
        ...
        self._lda_W = torch.tensor(
            W, dtype=torch.float32
        ).to(self._lda_W.device)
        ...
        def _cohen_d(a, b):
            return abs(a.mean() - b.mean()) / (
                np.sqrt((a.std()**2 + b.std()**2) / 2.0) + 1e-8
            )
        ...
        # LDA 학습 결과를 화면에 출력한다
        print(f"  [LDA] 3 binary directions fitted (F={self.feature_dim}->K=3)")
        # 방향 1의 분리도와 큐비트 할당을 출력한다
        print(f"  [LDA] Dir1 (LONG  vs HOLD)    : Cohen d = {d1:.4f}  "
              f"{'[OK]' if d1 > 0.1 else '[WEAK]'}  <- Qubit 1")
        # 방향 2의 분리도와 큐비트 할당을 출력한다
        print(f"  [LDA] Dir2 (SHORT vs HOLD)    : Cohen d = {d2:.4f}  "
              f"{'[OK]' if d2 > 0.1 else '[WEAK]'}  <- Qubit 2")
        # 방향 3의 분리도와 큐비트 할당을 출력한다
        print(f"  [LDA] Dir3 (HOLD  vs L+S)     : Cohen d = {d3:.4f}  "
              f"{'[OK]' if d3 > 0.1 else '[WEAK]'}  <- Qubit 3")
        # 각 클래스(LONG, SHORT, HOLD)의 샘플 수를 출력한다
        for label, mask in [("LONG", y==1), ("SHORT", y==-1), ("HOLD", y==0)]:
            print(f"         {label}: {int(mask.sum()):,} samples")

    def _lda_project(self, x_norm: torch.Tensor) -> torch.Tensor:
        """
        학습된 LDA 방향으로 피처 투영.
    ...
    def _edmd_decompose(
        ...
        if T < 4:
            return self._eigen_decompose(self._covariance_matrix(x_norm))
        ...
        G = torch.bmm(X.transpose(1, 2), X) / max(T1, 1)
        # 교차 상관 행렬: A = X^T × X_next / T1  [B, F, F]
        A = torch.bmm(X.transpose(1, 2), X_next) / max(T1, 1)

        # RMT 노이즈 제거 옵션이 켜져 있으면 그람 행렬의 노이즈를 제거한다
        if self.use_rmt:
            G = self._marchenko_pastur_denoise(G, T1)
        ...
        top_eig_abs = eigenvalues.abs().gather(1, top_idx).detach()
    ...
    def _extract_phase(self, c_kt: torch.Tensor) -> torch.Tensor:
        """
        ...
        zero_pad = torch.zeros(
            c_kt.shape[0], 1, c_kt.shape[2],
            dtype=c_kt.dtype, device=c_kt.device
        )                                                 # [B, 1, K]
        # 0 패딩을 앞에 붙여서 원래 시퀀스 길이를 유지한다
        delta_c_kt = torch.cat([zero_pad, delta], dim=1) # [B, T, K]
    ...
    def forward(
        ...
        if x.shape[-1] != self.feature_dim:
            raise ValueError(
                f"입력 피처 차원 불일치: "
                f"expected {self.feature_dim}, got {x.shape[-1]}"
            )
        ...
        if self.use_lda and self._lda_fitted:
            ...
            if c_lda.shape[-1] < self.n_eigenvectors:
                # 추가로 필요한 차원 수를 계산한다
                n_extra = self.n_eigenvectors - c_lda.shape[-1]
                # EDMD로 나머지 고유벡터를 구한다
            ...
            # ── Koopman EDMD: 예측 가능성 기준 정렬 ──────────────────────
            # EDMD로 예측 가능한 모드(단위원에 가까운 고유값)를 선택한다
            ...
            if self.use_rmt:
                cov = self._marchenko_pastur_denoise(cov, x_norm.shape[1])
        ...
        return x_norm, c_kt, delta_c_kt, eigenvalues

    # ─────────────────────────────────────────────────────────────────────
    # 설명 가능성 유틸리티
    # ─────────────────────────────────────────────────────────────────────

    def explained_variance_ratio(self) -> Optional[torch.Tensor]:
        """
    ...
    def qubit_labels(self) -> list[str]:
        ...
        if self.use_lda and self._lda_fitted:
            ...
            labels = [
                ...
                "Qubit-3 Volatility Phase  (v₃: ATR, Bollinger)",
            ]
        # n_eigenvectors 개수만큼만 잘라서 반환한다
        return labels[: self.n_eigenvectors]
    ...
    def summary(self) -> str:
        ...
        # 추출된 고유벡터 수를 추가한다
        lines.append(f"  Eigenvectors : {self.n_eigenvectors}")
        ...
        if evr is not None:
            lines.append("  Explained Variance Ratio (PCA):")
            for i, (label, r) in enumerate(
                zip(self.qubit_labels(), evr.tolist())
            ):
                # 각 고유벡터(큐비트)가 전체 분산의 몇 퍼센트를 설명하는지 출력한다
                lines.append(f"    [{i+1}] {label} → {r*100:.2f}%")
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("  SpectralDecomposer — 빠른 자가 검증 (Self-Test)")
    print("  마스터플랜 Part 2.1 수식 반영 확인")
    print("=" * 60)
    ...
    # ── 1. 동적 공분산 모드 (기본) ───────────────────────────────────────
    print("\n[Mode 1] Dynamic Covariance Basis")
    ...
    x = torch.randn(8, 60, 27, device=device)
    # 전체 파이프라인을 실행한다
    ...
    # 투영값의 모양을 출력한다
    print(f"  c_kt shape      : {tuple(c_kt.shape)}")
    # 위상 변화율의 모양을 출력한다
    print(f"  delta_c_kt shape: {tuple(delta_c_kt.shape)}")
    # 고유값의 모양을 출력한다
    print(f"  eigenvalues     : {tuple(eigenvalues.shape)}")
    ...
    for label, r in zip(decomposer.qubit_labels(), evr.tolist()):
        print(f"    {label} → {r*100:.2f}%")
    ...
    # 투영값의 모양을 출력한다
    print(f"  ✓ c_kt shape: {tuple(c_kt_l.shape)}")
    print(f"  ✓ eigenvalues: None (as expected)")

    # 전체 요약을 출력한다
    print()
    print(decomposer.summary())

    print("\n  ✓ 모든 검증 통과! Phase 1 구현 완료.")
    print("=" * 60)
